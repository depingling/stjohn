...
<td class="text"><div >
   ...
   &nbsp;
   <% } else { %>
   <bean:write name="sciD" property="product.manufacturerName"/>&nbsp;
   <% } %>
 </div></td>
<td class="text"><div >
    <bean:write name="sciD" property="product.manufacturerSku"/>&nbsp;
</div></td>
<% } %>
...
<input type="hidden"
 ...
 value="<%=ClwI18nUtil.getCurrency((String) session.getAttribute(Constants.CATALOG_LOCALE)).getGlobalCode()%>">
...
<!-- UNSPC code -->
<input type="hidden"
 name="NEW_ITEM-MATGROUP[<%=IDX.intValue() + 1%>]"
 value='<bean:write name="sciD" property="product.unspscCd"/>' >
...
String itemLink1 = "shop.do?action=item&source="+sourceStr+"&itemId="
  +itemId+"&qty="+sciD.getQuantity();
