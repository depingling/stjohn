...
public class LocateReportDistributorLogic {
 public static ActionErrors processAction(HttpServletRequest request,
    ...
    pForm = baseForm.getLocateReportDistributorForm();
 ...
 public static ActionErrors initSearch(HttpServletRequest request, StorePortalForm baseForm)
    ...
    LocateReportDistributorForm pForm = baseForm.getLocateReportDistributorForm();

    if(pForm==null) {
      pForm = new LocateReportDistributorForm();
      pForm.setLevel(1);
      baseForm.setLocateReportDistributorForm(pForm);
    }
 ...
 public static ActionErrors returnNoValue(HttpServletRequest request,
                      LocateReportDistributorForm pForm)
    	throws Exception
    {
    ActionErrors ae = new ActionErrors();
    HttpSession session  = request.getSession();
    session.removeAttribute(SessionAttributes.SEARCH_RESULT.REPORT_DISTRIBUTORS);
    return ae;
 }

  public static ActionErrors search(HttpServletRequest request,
            LocateReportDistributorForm pForm, StorePortalForm baseForm)
             throws Exception {
    ...
    if (pForm.getDataSourceType() != null && pForm.getDataSourceType().equals("DW")) {
  ...
  public static ActionErrors returnSelected(HttpServletRequest request,
    ...
    clearMultiDistributorFilterInSession(request, getFilterName(pForm));
    putMultiDistributorFilter2Session(request, retDV, getFilterName(pForm));
  ...
  public static ActionErrors clearFilter(HttpServletRequest request,StorePortalForm pForm) throws Exception{
      HttpSession session = request.getSession();
      LocateReportDistributorForm form = pForm.getLocateReportDistributorForm();
  ...
  public static ActionErrors setFilter(HttpServletRequest request,StorePortalForm pForm)
    ...
    pForm.setReturnLocateTypeCd(RefCodeNames.RETURN_LOCATE_TYPE_CD.DISTRIBUTOR);
    ...
    public static ActionErrors setFilterProgrammatically(HttpServletRequest request, 
        ...
        LocateReportDistributorForm locateForm = baseForm.getLocateReportDistributorForm();
        if (locateForm == null) {
            locateForm = new LocateReportDistributorForm();
        }
        ...
        baseForm.setLocateReportDistributorForm(locateForm);
        locateForm.setReturnLocateTypeCd(RefCodeNames.RETURN_LOCATE_TYPE_CD.DISTRIBUTOR);
        return new ActionErrors();
    }
    ...
    public static ActionErrors clearMultiDistributorFilterInSession(HttpServletRequest request,
                                                                    String filterName) {
    ...
    public static ActionErrors putMultiDistributorFilter2Session(HttpServletRequest request,
                               DistributorDataVector distributors, String filterName) {

    	HashMap hashMapFilter = null;
    	boolean canPutDistributors2Session = false;
    	...
    	if (hashMapFilter == null) {
			...
			canPutDistributors2Session = true;
		}

    	/// Putting the found accounts into proper session attribute
    	if (canPutDistributors2Session) {
    		LinkedList distributorList = new LinkedList();
    		if (distributors != null && distributors.size() > 0) {
    			for (int i = 0; i < distributors.size(); ++i) {
    				DistributorData distributor = (DistributorData)distributors.get(i);

                                if (filterName.equals(Constants.DW_DISTRIBUTOR_FILTER_NAME)) {
                                  distributorList.add(distributor.getBusEntity().getBusEntityId());
                                } else {
                                  distributorList.add(new Integer(distributor.getBusEntity().getBusEntityId()));
                                }

    			}
    		}
    		hashMapFilter.put(filterName, distributorList);
    	}

    	return errors;
    }

    private static String getFilterName (LocateReportDistributorForm pForm){
      String filterName = Constants.DISTRIBUTOR_FILTER_NAME;
      if (pForm != null) {
        if (pForm.getDataSourceType().equals("DW")) {
          filterName = Constants.DW_DISTRIBUTOR_FILTER_NAME;
        }
      }
      return filterName;
     }
