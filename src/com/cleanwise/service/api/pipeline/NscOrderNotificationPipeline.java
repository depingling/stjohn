...
public class NscOrderNotificationPipeline implements OrderPipeline {

    private  static final String className="OrderNotification";
    private static final String PUNCH_OUT_EMAIL_LABEL = "Punch Out Email (Yes/No):";
    private static final String PUNCH_OUT_EMAIL_TRUE = "Y";
    ...
    throws PipelineException 
    {
        ...
        try {
            ...
            boolean punchOutEmail = isPuncOutEmail(pCon, accountId);
            generateMessage(messageSB,
                pBaton.getOrderData(), pBaton.getShipToData(),
                pBaton.getOrderMetaDataVector(), oiDV,
                accountName, custMaj, locationNumber, addOnCharges, punchOutEmail);
...
//            eventEjb.addEventEmail(eventEmailData);
    ...
    private boolean isPuncOutEmail(Connection pConn, int pAccountId)
            throws Exception {
        DBCriteria dbCrit = new DBCriteria();
        dbCrit.addEqualTo(PropertyDataAccess.BUS_ENTITY_ID, pAccountId);
        dbCrit.addEqualTo(PropertyDataAccess.PROPERTY_TYPE_CD,
                RefCodeNames.PROPERTY_TYPE_CD.ACCOUNT_FIELD_CD);
        dbCrit.addEqualTo(PropertyDataAccess.SHORT_DESC, PUNCH_OUT_EMAIL_LABEL);
        dbCrit.addEqualTo(PropertyDataAccess.PROPERTY_STATUS_CD,
                RefCodeNames.PROPERTY_STATUS_CD.ACTIVE);
        PropertyDataVector pdv = PropertyDataAccess.select(pConn, dbCrit);
        if (pdv != null && pdv.size() > 0) {
            PropertyData propertyData = (PropertyData) pdv.get(0);
            String value = propertyData.getValue();
            if (Utility.isSet(value)
                    && value.length() > 0
                    && PUNCH_OUT_EMAIL_TRUE.equalsIgnoreCase(value.substring(0,
                            1))) {
                return true;
            }
        }
