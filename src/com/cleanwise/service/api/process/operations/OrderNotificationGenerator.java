...
public class OrderNotificationGenerator extends OrderNotificationGeneratorBase implements FileGenerator {
...
//        writeHeaderTXT(sb,data.getOrderInfo());
    ...
    public void writeBodyTXT(StringBuffer sb, ItemInfoViewVector items) {
        ...
        while (it.hasNext()) {
            ...
            colsArray.add(parseDelim(item.getCustCost().toString(),null, 10-TABLE_COLUMN_SPACE));
            colsArray.add(parseDelim(item.getQty().toString(),null, 5-TABLE_COLUMN_SPACE));
            colsArray.add(parseDelim(item.getQty().multiply(item.getCustCost()).toString(),null, 12-TABLE_COLUMN_SPACE));
    ...
    private void writeSummaryInfo(StringBuffer sb, OrderInfoView orderInfo) {
        nextLine(sb);
        int maxLenPh = "Misc Charge: ".length();
        int maxDigitinLine=12;
        line.append(align(normalizeString("SubTotal: ",' ',maxLenPh,LEFT)
                +normalizeString(orderInfo.getSubtotal().toString(),' ',maxDigitinLine,LEFT),
                RIGTH));
