 ...
 *@author Deping
 */
public class Inbound810Super extends InboundEdiSuper
{
	private static final Logger log = Logger.getLogger(Inbound810Super.class);
	protected static final String FREIGHT_SAC = "D240";
	protected static final String MISC_SAC = "D500";
	protected static final String OTHER_CHARGE = "F050"; 
	protected static final String SALES_TAX_SAC = "H740";
	protected static final String SALES_TAX_STATE_SAC = "H770";
	protected static final String DISCOUNT_SAC = "C310";

	// Processing variables
	protected InvoiceRequestData reqInvoice = new InvoiceRequestData();
	...
	protected String[] freightSKUs = null;
	protected BigDecimal excludedTotal = new  BigDecimal(0);

	protected String distAccRefNum;
		...
		try {
			...
			for (int i = 0; i <  numberOfSegmentsInVector; i++)
			{
				...
				if (Utility.isSet(de.get())) {
					...
					String identification = segment.getDataElement(2).get();
					...
					// the shipment ID.
					if ("SI".equals(identQualifier)) {
						currInvoice.setDistShipmentNum(identification);
					}
					else if ("DI".equals(identQualifier)) { // Distributor Identifier, uniquly identifies
						// a distributor for anyone with multiple distributors
						//under 1 umbrella which we want to validate off of.

						distributorCompanyCode = identification;
					} else if("D2".equals(identQualifier)) {
						currInvoice.setDistOrderNum(identification);
					} else if("ZZ".equals(identQualifier)) {
						currInvoice.setErpPoNum(identification);
					}
				}
			}
	...
	{
		Loop loop;
		boolean shipToFound = false, shipFromFound = false;
			...
			for (int i = 0; i <  numberInVector; i++)
			{
				...
				if (Utility.isSet(de.get())) {
					if (code.equalsIgnoreCase("ST")) {
						...
						extractHeaderN1N4(code, loop);
					} else if (code.equalsIgnoreCase("BT")){
						// check for account identifier (eg. eastern bag)
						if(RefCodeNames.ACCOUNT_IDENTIFIER_INBOUND.ACCT_IDENTIFIER_IN_N1_LOOP.equals(
							tp.getAccountIdentifierInbound())){

							distAccRefNum = de.get().trim();
						}
	...
	public void extractHeaderN1N3(String id, Loop inLoop)  throws OBOEException {
		...
		try {
			...
			for (int i = 0; i <  1; i++) {
				...
				DataElement de = segment.getDataElement(1);       // 166 Address Information
				if(de != null && Utility.isSet(de.get())){
					if (id.equals("ST")) {
	...
	public void extractHeaderN1N4(String id, Loop inLoop)  throws OBOEException {
		...
		log.info("X1.2 extractHeaderN1N4\n  id=" + id + "\n  Loop=" + inLoop.get() );
	...
	throws OBOEException
	{
		...
		for (int i = 0; i <  numberInVector; i++)
		{
			...
			if ( de == null ) {
				log.info("2005.6.28 (de is null) segment missing "
						+ ", segment=" + segment );
			...
			if (de != null && Utility.isSet(de.get())) {
				currDetail.setItemReceivedCost(getItemReceivedCost(de.get()));
			...
			if (distItemSkuNum != null){
				if (Utility.isInArray(distItemSkuNum, excludedSKUs)){
					...
					continue;
				}else if (Utility.isInArray(distItemSkuNum, freightSKUs)){
					currInvoice.setFreight(Utility.addAmt(currInvoice.getFreight(),lineTotal));
					continue;
				}else{
	...
	throws OBOEException
	{
		...
		if (Utility.isSet(de.get())) {
			...
			controlTotalSumInvoice.subtract(excludedTotal);
					...
					if ("C".equals(indicator) || "A".equals(indicator) || "P".equals(indicator)||
						...
						if (Utility.isSet(de.get())) {
							...
							if (code.equals(FREIGHT_SAC)) {
								currInvoice.setFreight(Utility.addAmt(currInvoice.getFreight(),getAmountFromHeaderSACSegment(segment,de)));
							}else if (code.equals(MISC_SAC)) {
								currInvoice.setMiscCharges(Utility.addAmt(currInvoice.getMiscCharges(),getAmountFromHeaderSACSegment(segment,de)));
							}else if (code.equals(OTHER_CHARGE)) {
								currInvoice.setMiscCharges(Utility.addAmt(currInvoice.getMiscCharges(),getAmountFromHeaderSACSegment(segment,de)));
							}else if (code.equals(SALES_TAX_SAC)) {
								BigDecimal tax = Utility.addAmt(getAmountFromHeaderSACSegment(segment,de), currInvoice.getSalesTax());
								currInvoice.setSalesTax(tax);
							}else if (code.equals(SALES_TAX_STATE_SAC)) {
									BigDecimal tax = Utility.addAmt(getAmountFromHeaderSACSegment(segment,de), currInvoice.getSalesTax());
									currInvoice.setSalesTax(tax);
							}else if (code.equals(DISCOUNT_SAC)) {
								currInvoice.setDiscounts(Utility.addAmt(currInvoice.getDiscounts(),
										Utility.toNegative(getAmountFromHeaderSACSegment(segment,de))));
							}else
							{   // this error  message  will be added, only if check in InboundEdiHandler.java is not carried out
								...
								setValid(false);
								// log.info("Error: Unknown type Service, Promotion, Allowance, or Charge Code");
		...
		for(int subI = 5;subI<=segCt;subI++){
			...
			if (de != null && Utility.isSet(de.get())) {
				try{
					...
					log.info("Found something non-numberic at or after position 4");
				}
			}
		}
	...
	public void processTransaction() throws Exception
	{
		...
		if(distAccRefNum != null){
			reqInvoice.setDistributorAccountRefNum(distAccRefNum);
		}
		...
		boolean matchByVendorOrderNum = reqInvoice.getMatchPoNumType().equals(RefCodeNames.MATCH_PO_NUM_TYPE_CD.VENDOR_ORDER_NUM);
		String poNum = matchByVendorOrderNum ? currInvoice.getDistOrderNum() : currInvoice.getErpPoNum();
		orderD = getTranslator().getOrderDataByPoNum(poNum, null, distAccRefNum, reqInvoice.getMatchPoNumType());		
		...
		getTransactionD().setKeyString("ErpPoNum: " + currInvoice.getErpPoNum()
		...
		ediHandler.appendIntegrationRequest(reqInvoice);
	}
