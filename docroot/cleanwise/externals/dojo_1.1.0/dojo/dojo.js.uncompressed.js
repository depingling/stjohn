...
dijit.popup = new function(){
	...
	this.open = function(/*Object*/ args){
        ...
        if (padding) {
            var best = around ?
                       this.placeOnScreenArroundPadding(wrapper, around, orient, widget.orient ? dojo.hitch(widget, "orient") : null,padding) :
    ...
    this.placeOnScreenArroundPadding = function(
    /* DomNode */        node,
                         /* DomNode */        aroundNode,
                         /* Object */        aroundCorners,
                         /* Function */        layoutNode, padding) {
        ...
        for (var nodeCorner in aroundCorners) {
            choices.push({
                ...
                corner: aroundCorners[nodeCorner],
                pos: {
                    x: aroundNodePos.x + padding.right - padding.left + (nodeCorner.charAt(1) == 'L' ? 0 : aroundNodeW),
                    y: aroundNodePos.y + padding.top - padding.bottom + (nodeCorner.charAt(0) == 'T' ? 0 : aroundNodeH)
                }
            });
        }
...
if(!dojo._hasResource["clw.NewXpedx.Calendar"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
dojo._hasResource["clw.NewXpedx.Calendar"] = true;

 dojo.provide("clw.NewXpedx.Calendar");
