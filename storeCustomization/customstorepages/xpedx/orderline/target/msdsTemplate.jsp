...
<%String submitUrl =submitBase+"?action=showDoc";%>
  ...
  <tr>
  <td colspan="8" class="msdscharthead"><div class="fivemargin">
  ...
  <logic:iterate id="item" name="MSDS_FORM" property="cartItems"
   ...
   type="com.cleanwise.service.api.value.ShoppingCartItemData">
     ...
     <% if(theForm.isCategoryChanged(((Integer)kkk).intValue())) { %>
       <tr>
       <td class="shopcategory" colspan = "8"><bean:write name="item" property="categoryPath"/></td>
       </tr>
     <% } %>
     <% } %>
     ...
     <td class="text" align="center">
       ...
       if("MSDS".equals(theForm.getDocType())) {
           ...
           //docUrl=item.getProduct().getMsds(); // old code
       }
       else if("SPEC".equals(theForm.getDocType())) {
         docUrl=item.getProduct().getSpec();
       }
       else if("DED".equals(theForm.getDocType())) {
  ...
  <tr>
  <td colspan="8" class="msdscharthead" align="center">&nbsp</td>
  </tr>
</table>
</td></tr>
<tr><td>
