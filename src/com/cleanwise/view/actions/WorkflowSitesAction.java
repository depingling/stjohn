 ...
 */
public final class WorkflowSitesAction extends ActionSuper {
    ...
    throws IOException, ServletException {
        ...
        try {
            ...
            if (action.equals(searchStr) ||
                ...
                WorkflowMgrLogic.searchForSites(request, form);
                return (mapping.findForward("success"));
            }
            else if (action.equals("sort")) {
                WorkflowMgrLogic.sortSites(request, form);
                return (mapping.findForward("success"));
            }
            else if (action.equals(saveStr)) {
		ActionErrors ae = WorkflowMgrLogic.linkSites
		    (request, form);
		if (ae.size() > 0) {
		    saveErrors(request, ae);
                ...
                ActionErrors ae = WorkflowMgrLogic.assignAllSites
		    (request, form);
		if (ae.size() > 0) {
		    saveErrors(request, ae);
		}
                ...
                ActionErrors ae = WorkflowMgrLogic.fetchCurrentSites
		    (request, form);
		if (ae.size() > 0) {
		    saveErrors(request, ae);
		}
                ...
                WorkflowMgrLogic.initSites(request, form);
                return (mapping.findForward("success"));
            }
