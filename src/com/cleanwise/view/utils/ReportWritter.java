...
//                  workbook = new XSSFWorkbook();
 ...
 //for POI 3.5      CreationHelper createHelper = workbook.getCreationHelper();
  ...
  //for POI 3.5                       style.setDataFormat(createHelper.createDataFormat().getFormat(columnFormat));
 ...
 //for POI 3.5                      style.setDataFormat(createHelper.createDataFormat().getFormat(columnFormat));
	...
	public static void writeExcelReport(GenericReportResultView repRes,
			...
			ReportRequest pRepRequest) throws Exception {
		int columnCount = repRes.getColumnCount();
                GenericReportColumnViewVector header = repRes.getHeader();
...
// for POI 3.5          CreationHelper createHelper = pWorkbook.getCreationHelper();
...
// for POI 3.5                                  style.setDataFormat(createHelper.createDataFormat().getFormat(cellFormat));
...
/*                        if (ii % 1000 == 0) {
                            loginfo("+========= Memory Usage =====" + ii);
                            loginfo("! " + MemoryStatistics.getUsed());
                            loginfo("! " + MemoryStatistics.getCommited());
                            loginfo("! " + MemoryStatistics.getMax());
                            loginfo("+================================");
                          } */
			}
		}
...
//                    String fmt = strNew +"##,##0.00";
	...
	public static void writeCSVReport(GenericReportResultViewVector repResv,
			OutputStream pOut, ReportRequest pRepRequest) throws Exception {
	...
	public static void writeExcelReportMulti(
			GenericReportResultViewVector repResults, OutputStream pOut,
			ReportRequest pRepRequest, String xlsType ) throws Exception {
 ...
 //       for (int ii = 0; ii < repResults.size(); ii++) {
    ...
    private final  static class IndexedAligns  {
     ...
     public  static class CustomTableModel extends AbstractTableModel  {
               ...
               public void setColumnNames(ArrayList pColumnNames)
               {
                 columnNames = new String[pColumnNames.size()];
                 for (int i = 0; i < pColumnNames.size(); i++) {
                   setColumnName(i, (String)pColumnNames.get(i));
                 }
               }
               public void setValueAt(Object pValue, int rowIndex, int columnIndex) {
                 Object val = pValue;
                 if (pValue instanceof HashMap){
                   val = ((HashMap)pValue).get("BOLD") ;
                 }

                 this.data[rowIndex][columnIndex] = val;
               }
