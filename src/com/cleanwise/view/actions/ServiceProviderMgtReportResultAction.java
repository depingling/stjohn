 ...
 */
public final class ServiceProviderMgtReportResultAction extends ActionSuper {
  ...
  public ActionForward performSub(
	  ...
	  if (action.equals("init") || action.equals("get_delivery_sched")) {
	      ...
	      ServiceProviderMgtReportLogic.downloadGenericReport
		  (request,response,form, rr);
	      return null;
          } else if ("Download to Excel".equals(action)) {
              ServiceProviderMgtReportLogic.downloadGenericReport(request,response,form, rr);
              return null;
          } else if ("Download to PDF".equals(action)) {
              ServiceProviderMgtReportLogic.downloadPDFReport(request,response,form, rr);
              return null;
          }

          // Old mechanism to export the current report to xcel.
          else  if (action.indexOf("xport") == 1 ) {
            ActionErrors ae = ServiceProviderMgtReportLogic.exportOrderReport(request, response, (ServiceProviderMgtReportForm)form);
            if (ae.size() > 0) {
              saveErrors(request, ae);
              return (mapping.findForward("display"));
            }
              ...
              ActionErrors ae = ServiceProviderMgtReportLogic.getGenericReportPage(request,form,action);
              if (ae.size() > 0) {
                  saveErrors(request, ae);
                  return (mapping.findForward("display"));
              }
