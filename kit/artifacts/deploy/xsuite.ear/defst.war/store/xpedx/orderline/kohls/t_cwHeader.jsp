...
<%-- hide cut off date when discretionary cart available--%>
<%
if(!siteData.hasDiscretionaryCartAccessOpen()){
if (siteData.hasInventoryShoppingOn()) {%>
    <app:storeMessage key="shop.header.text.scheduledShopping"/>,
<% } %>
...
<td align="right">
        <a class="headernavlinkstart" href="../userportal/customerhome.do?hp_action=init"><app:custom pageElement="pages.home.button"/></a>
</td>
<%
              ...
              <% if (ShopTool.hasDiscretionaryCartAccessOpen(request)) { %>
                <tr>
                   <td align="left">
                        <a href="../store/shoppingcart.do" class="linkButton">
                            ...
                            <app:storeMessage key="global.label.discretionaryCart"/>
                        </a>
                    </td>
                ...
                <tr>
                    ...
                    <td align="left" class="text">
                        <div class="fivemargin">
   <%
   ShoppingCartData mySc = (ShoppingCartData) session.getAttribute(Constants.INVENTORY_SHOPPING_CART);
   int myuom = mySc.getTotalItemQuantity();
   int myqty = mySc.getItemsQty();
   %>
   
                          <%--     <%=ClwI18nUtil.getShoppingItemsString(request,(ShoppingCartData) session.getAttribute(Constants.INVENTORY_SHOPPING_CART))%> --%>
<%=myqty%>&nbsp;<app:storeMessage key="shoppingCart.text.items"/>,<%=myuom%>&nbsp;<app:storeMessage key="shoppingCart.text.units"/>
                    </td>
