...
public class OutboundNetwork850CsvTabPurchaseOrder2 extends InterchangeOutboundSuper implements OutboundTransaction{
...
//    private static final BigDecimal ZERO = new BigDecimal(0);
    ...
    public OutboundNetwork850CsvTabPurchaseOrder2(){
...
/////////////////////                
        ...
        if(Utility.isSet(comments)) {
        	...
        	for (int i = 0; i < len; i+=62){
        		int j = Math.min(len, i+62);
        		orderDoc.append("I"); //instructions
                orderDoc.append(oNum); //Order number
                orderDoc.append(Utility.padLeft(rowNum++, '0', 2)); //Sequence
    ...
    if(Utility.isSet(currOrder.getOrderContactName())){
    	contactStr = currOrder.getOrderContactName() + " ";
    }
    if(Utility.isSet(currOrder.getOrderContactPhoneNum())){
    	contactStr = contactStr + currOrder.getOrderContactPhoneNum() + " ";
    }
    if(Utility.isSet(currOrder.getOrderContactEmail())){
    	contactStr = contactStr + currOrder.getOrderContactEmail();
    }
    contactStr = contactStr.trim();
    if(Utility.isSet(contactStr)) {
    	int len = contactStr.length();
    	for (int i = 0; i < len; i+=62){
    		int j = Math.min(len, i+62);
    		orderDoc.append("I"); //instructions
            orderDoc.append(oNum); //Order number
            orderDoc.append(Utility.padLeft(rowNum++, '0', 2)); //Sequence
            String temp = Utility.padRight(contactStr.substring(i, j), ' ', 62);;
            orderDoc.append(temp);
            orderDoc.append("\r\n"); 
    	} 
    }
  ...
  /*
   * Insert checkout data fields to the output file. 
   * Output file should always contain 10 entries for checkout fields. If a 
   * checkout data field is null or empty or not defined yet, then entry for 
   * that line will look like this '@+@@+@' 
   */
  for(int i=1; i<=10; i++){
		String temp="";
		/*
		 * Find out whether i-th checkout data field is defined or not, if so then 
		 * get the user defined name for i-th field.
		 */
		String propName ="";
		PropertyDataVector accountPropDV = currOutboundReq.getAccountProperties();
		for(Object accountPropTmp : accountPropDV){
			PropertyData accountProp = (PropertyData) accountPropTmp;
	  	if((accountProp.getPropertyTypeCd().equalsIgnoreCase("CHECKOUT_FIELD_CD"))){
	    		if(Utility.isSet(accountProp.getShortDesc())){
	    			if(accountProp.getShortDesc().contains(i+"Tag")){  
	        		propName = accountProp.getValue();  //user defined name for the i-th checkout data field.
	        		break;
	    			}
	    		}
	  	}
		...
		/*
		 * if i-th checkout data field is defined by user, then get the value for that field.
		 */
		OrderPropertyDataVector orderPropDV = currOutboundReq.getOrderPropertyDV();
		if(Utility.isSet(propName)){
  		 if(orderPropDV != null) {
  			 for(Object orderPropTmp : orderPropDV){
  				 OrderPropertyData orderProp = (OrderPropertyData)orderPropTmp;
        	 if(orderProp.getShortDesc().equalsIgnoreCase(propName)){
        		 String propValue = orderProp.getValue();
        		 int len = propValue.length();
        		 int j = Math.min(len, 56);
        		 temp = propValue.substring(0, j);
        		 break;
        	 }
         }
  		 }
  ...
  //end of inserting checkout data fields        
