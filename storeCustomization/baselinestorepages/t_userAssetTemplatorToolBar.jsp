...
<%
    ...
    String actionLink03 = request.getParameter("actionLink03");
    String tabName03 = request.getParameter("tabName03");
    String item03 = Utility.strNN(request.getParameter("item03"));
    String secondaryItems03 = Utility.strNN(request.getParameter("secondaryItems03"));
    String itemLable03 = request.getParameter("itemLable03");

    if (actionLink03 != null) {
        char sp03 = (actionLink03.indexOf("?") >= 0) ? '&' : '?';
        actionLink03 += sp03 + "tabs=" + tabName03 + "&display=" + item03;
    }
    
    String actionLink04 = request.getParameter("actionLink04");
    String tabName04 = request.getParameter("tabName04");
    String item04 = Utility.strNN(request.getParameter("item04"));
    String secondaryItems04 = Utility.strNN(request.getParameter("secondaryItems04"));
    String itemLable04 = request.getParameter("itemLable04");

    if (actionLink04 != null) {
        char sp04 = (actionLink04.indexOf("?") >= 0) ? '&' : '?';
        actionLink04 += sp04 + "tabs=" + tabName04 + "&display=" + item04;
    }
    ...
    boolean isWOAdministrator = appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.ASSET_WO_VIEW_ALL_FOR_STORE);
    ...
    <td class="toolbar1" align="left" colspan="2">
        <table style="padding-bottom: 0em;" cellpadding="0" cellspacing="0" class="headernavtable" style="border-collapse: collapse;">
            <tr>
                ...
                <%}%>
                <%if((secondaryItems03 != null || item03!=null) && isAssetAdministrator && isWOAdministrator){%>
                    <%
                    boolean currPage = false;
                    if(item03.indexOf(display) >= 0 ||
                            secondaryItems03.indexOf(display)>0) {

                        currPage = true;
                        headerLabelSection = itemLable03;
                        toolSecondaryToolBar=toolSecondaryToolBar02;

                    }
                    %>
                	<td class="toolbar1" valign="bottom">
                	  <a class=secondaryNavButton<%=(currPage)? "Curr": ""%> href="<%=actionLink03%>"><%if(itemLable03!=null && itemLable03.length()>0){%><app:storeMessage key="<%=itemLable03%>"/><%}%></a>
                    </td>
                <%}%>
                <%if((secondaryItems04 != null || item04!=null) && isAssetAdministrator && isWOAdministrator){%>
                    <%
                    boolean currPage = false;
                    if(item04.indexOf(display) >= 0 ||
                            secondaryItems04.indexOf(display)>0) {

                        currPage = true;
                        headerLabelSection = itemLable04;
                        toolSecondaryToolBar=toolSecondaryToolBar02;

                    }
                    %>
                	<td class="toolbar1" valign="bottom">
                	  <a class=secondaryNavButton<%=(currPage)? "Curr": ""%> href="<%=actionLink04%>"><%if(itemLable04!=null && itemLable04.length()>0){%><app:storeMessage key="<%=itemLable04%>"/><%}%></a>
                    </td>
                <%}%>
            </tr>
        </table>
