...
<%String tutURL=ClwCustomizer.getRootFilePath(request, "customtrainingflash.jsp");%>
    ...
    <td><img class=mainheadernavspacer src='<%=ClwCustomizer.getSIP(request,"cw_global_main_menu_spacer.gif")%>' HEIGHT="64"/></td>
    <td class=mainheadernav>
        <a class=mainheadernav href="../userportal/msdsTemplate.do?tabs=safetyToolBar&display=msdsTemplate">
            <img src='<%=ClwCustomizer.getSIP(request,"cw_global5off.gif")%>' border="0" name="cw_global3"  HEIGHT="50"
			><br><img src='<%=ClwCustomizer.getSIP(request,"cw_hline.gif")%>' border="0" height="1" width="100%"
			><br><app:storeMessage key="template.jd.bsc.menu.main.msdsSheets"/></a>
    </td>
    ...
    <td class=mainheadernav>
        <a class=mainheadernav href="../userportal/templator.do?display=custommarketing&tabs=marketingToolBar">
			...
			><br><app:storeMessage key="template.jd.bsc.menu.main.operations"/></a>
    </td>
	<% } %>
    <% if (Utility.isTrue(allowAssetManagement)
	       && appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.ASSET_USER)){
