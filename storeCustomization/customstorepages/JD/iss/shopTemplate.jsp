...
<bean:define id="numCols" value="11"/>
<%
...
<logic:greaterThan name="SHOP_FORM" property="categoryPathSize" value="0">
   ...
   <logic:iterate id="category" name="SHOP_FORM" property="categoryPath"
    indexId="kkk"
    type="com.cleanwise.service.api.value.CatalogCategoryData">
     <bean:define id="categoryId" name="category" property="catalogCategoryId"/>
     <br>
     <%= shift %>
     <% shift+="&nbsp;&nbsp;&nbsp;&nbsp;"; %>
     <% int sizeInt=((Integer)size).intValue();
      if(sizeInt-1>kkk.intValue()) {%>
     <a href="javascript:  document.forms[0].elements['navigateCategoryId'].value='<%=categoryId%>'; document.forms[0].submit();" ><bean:write name="category" property="catalogCategoryShortDesc"/></a>
       
     </a>
     <% } else { %>
       <b><bean:write name="category" property="catalogCategoryShortDesc"/></b>
     <% } %>
    </logic:iterate>
</logic:greaterThan>
--%>
...
<logic:greaterThan name="SHOP_FORM" property="categoryListSize" value="0">
  <logic:iterate id="selectCategory" name="SHOP_FORM" property="categoryList"
    indexId="kkk"
    type="com.cleanwise.service.api.value.CatalogCategoryData">
    <bean:define id="categoryId" name="selectCategory" property="catalogCategoryId"/>
    <br>
    <%= shift %>
    <a href="javascript:  document.forms[0].elements['navigateCategoryId'].value='<%=categoryId%>'; document.forms[0].submit();" >
       <bean:write name="selectCategory" property="catalogCategoryShortDesc"/>
    </a>
   </logic:iterate>
</logic:greaterThan>
<br>&nbsp;
  ...
  <logic:iterate id="item" name="SHOP_FORM" property="cartItems"
   ...
   type="com.cleanwise.service.api.value.ShoppingCartItemData">
     ...
     <% if(theForm.isCategoryChanged(((Integer)kkk).intValue())) { %>
       <tr>
       <td class="shopcategory" colspan = "11"><bean:write name="item" property="categoryName"/>
</td>
       </tr>
     <% } %>
     <% } %>
