 ...
 * KnowledgeNoteDetail or updates an existing KnowledgeNoteDetail.
 */
public final class KnowledgeOpNoteDetailAction extends ActionSuper {
  ...
  public ActionForward performSub(
    ...
    if( ("edit".equals(action) || "view".equals(action)) && ("".equals(noteId) || "0".equals(noteId))) {
        action = "add";
    }
    ...
    try {
      ...
      if (action.equals("add")) {
        KnowledgeOpLogic.addKnowledgeNote(request, form, knowledgeId);
        return (mapping.findForward("display"));
      }

      // Edit an existing knowledgeDetail.
      else if (action.equals("edit") || action.equals("view") ) {
        KnowledgeOpLogic.editKnowledgeNote(request, form, knowledgeId, noteId);  
        return (mapping.findForward("display"));
      }
      ...
      // Save the new or updated knowledgeDetail.
        ...
        ActionErrors ae = KnowledgeOpLogic.saveKnowledgeNote(request, form);
        if (ae.size() > 0 ) {
