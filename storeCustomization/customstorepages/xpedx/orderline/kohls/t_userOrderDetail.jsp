...
function viewPrinterFriendly(oid) {
  var loc = "printerFriendlyOrder.do?action=orderStatus&orderId=" + oid + "&showMfg=n&showMfgSku=n&showPar=n";
...
<% } %>
        ...
        <tr>
<td class="text"><b><app:storeMessage key="shop.orderStatus.text.source"/></b></td>
...
<td><bean:write name="ORDER_OP_DETAIL_FORM" property="orderPlacedBy"/>
 (<bean:write name="ORDER_OP_DETAIL_FORM" property="orderStatusDetail.orderDetail.addBy"/>)</td>
        </tr>
<%
...
<%--    <td colspan="2" class="text"><b><app:storeMessage key="label.customer.order.notes" arg0="<%=accountName%>" />:</b><br>--%>
    <td colspan="2" class="text"><b><app:storeMessage key="label.customer.order.notes" arg0="Kohl's" />:</b><br>
  <html:textarea name="ORDER_OP_DETAIL_FORM" property="customerComment" rows="3" cols="55"/>
<%
 String cc = theForm.getCustomerComment();
%>
...
if(((OrderItemData)item.getOrderItem()).getDistErpNum()!= null
        && !((OrderItemData)item.getOrderItem()).getDistErpNum().equals(lastDistErpPoNum)){ //display po header text

    lastDistErpPoNum = ((OrderItemData)item.getOrderItem()).getDistErpNum();
  %>
     <tr bgcolor="<%=ClwCustomizer.getEvenRowColor(session,(Integer)oidx)%>">
      ...
      <td colspan="4">
        <b><app:storeMessage key="shop.orderStatus.text.purchaseOrder"/></b>
            <logic:present name="item" property="orderItem.outboundPoNum">
             <bean:write name="item" property="orderItem.outboundPoNum"/>
           </logic:present>
      </td>
...
<% if (ShopTool.getCurrentSite(request).isAnInventoryOrderItem(item.getOrderItem().getItemId())){ %>
...
%>
   ...
   <tr>
   <td>&nbsp;</td>
   <td colspan="<%=(colqty-6)%>" class="rep_line">
  <%
  ...
  <% } %>
   ...
   <td colspan="<%=(colqty-5)%>" class="rep_line"><bean:write name="shoppingEntry" property="addDate"/></td>
...
<html:button styleClass="store_fb" property="action"  onclick='<%=rejectloc%>' >
  <app:storeMessage key="global.action.label.reject" /></html:button>
<% if(!toBeConsolidatedFl) { %>
<html:button styleClass="store_fb" property="action" onclick='<%=approveloc%>' >
  <app:storeMessage key="global.action.label.approve" /></html:button>
<% } %>
<%if(!consolidatedOrderFl){ %>
<html:button styleClass="store_fb" property="action"   onclick='<%=modifyloc%>'>
  <app:storeMessage key="shop.orderStatus.text.modify" /></html:button>
<% } %>
...
<html:button styleClass="store_fb" property="action"
   value='<%=ClwI18nUtil.getMessage(request,"global.action.label.reorder",null)%>'  onclick='<%=reorderloc%>'  />
--%>
<app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.RECEIVING%>">
