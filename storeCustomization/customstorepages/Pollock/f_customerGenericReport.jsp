...
<%
    ...
    if (xlateReportname.indexOf("Manufacturer Item Summary") >= 0 &&
        grcVV != null && grcVV.size() > 0) {
        String manufParam = "";
        String dateBeginParam = "";
        String dateEndParam = "";
        ...
        for (int ii = 0; ii < grcVV.size(); ii++) {
            ...
            if (Utility.isSet(value)) {
                if (name.startsWith("MANUFACTURER")) {
                    int manufId = 0;
                    try {
                        manufId = Integer.parseInt(value);
                    } catch (Exception e) {
                        manufId = 0;
                    }
                    if (manufId > 0) {
                        BusEntityDataVector manufDV = (BusEntityDataVector)session.getAttribute("Report.all.manufacturer.for.store");
                        if (manufDV != null) {
                            for (Iterator iter = manufDV.iterator(); iter.hasNext();) {
                                ...
                                if (manufD.getBusEntityId() == manufId ) {
                                    manufParam = manufD.getShortDesc();
                                    break;
                                }
                            }
                        }
                    }
                } else if (name.equals("BEG_DATE")) {
                    dateBeginParam = value;
        ...
        if (Utility.isSet(manufParam)) {
%>
            <tr>
                <td><b>Manufacturer Name:</b>&nbsp; <%=manufParam%></td>
            </tr>
<%
        }
        // Items parameter
...
%>
        <tr><td><b>Date Period Selected:</b> <%=dateBeginParam%> - <%=dateEndParam%></td></tr>
...
<%=tdRight%>
<%--  
<i18n:formatDate value="<%=value%>" locale="<%=thisUserLocale%>"/>&nbsp;</td>
--%>
