...
/**
 * <code>StoreDistMgrTerrLogic</code>
 *
 * @author     Veronika Denega
 * @created    June 3, 2006
 */
public class StoreDistMgrTerrLogic {
  ...
  public static void init(HttpServletRequest request,
    ...
    StoreDistMgrTerrForm sForm = (StoreDistMgrTerrForm) form;
  ...
  public static ActionErrors search(HttpServletRequest request,
    ...
    if (city != null && city.trim().length() > 0) {
      conditions.put("city", city.toUpperCase());
    }
  ...
  public static ActionErrors searchCounty(HttpServletRequest request,
                                          ActionForm form, Hashtable pConditions) throws Exception {
  ...
  public static ActionErrors saveCounty(HttpServletRequest request,
    ...
    for (int ii = 0; ii < betVV.size(); ii++) {
      ...
      for (; jj < selected.length; jj++) {
        String countyCd1 = selected[jj].substring(2);
    ...
    for (int ii = 0; ii < betVVUpdate.size(); ii++) {
      ...
      for (; jj < busEntityTerrFreightCd.length; jj++) {
        String countyCd1 = busEntityTerrFreightCd[jj].substring(2);
  ...
  public static ActionErrors saveZipCode(HttpServletRequest request,
    ...
    for (int ii = 0; ii < betVV.size(); ii++) {
      ...
      //is checked?
      String postalCode = betV.getPostalCode() + "^" + betV.getCity() + "^";
      int jj = 0;
    ...
    for (Iterator iter = betVVUpdate.iterator(); iter.hasNext(); ) {
      BusEntityTerrView betV = (BusEntityTerrView) iter.next();
      String postalCode = betV.getPostalCode();
      if (updateHt.containsKey(postalCode)) {
        BusEntityTerrView htBetV = (BusEntityTerrView) updateHt.get(postalCode);
        if (htBetV.getCheckedFl() != betV.getCheckedFl()) {
          ae.add("error", new ActionError("error.simpleGenericError",
                                          "The same zip code lines have different assign checkbox values. Zip code: " +
                                          postalCode));
          return ae;
        }
        if (!htBetV.getBusEntityTerrFreightCd().equals(betV.getBusEntityTerrFreightCd())) {
          ae.add("error", new ActionError("error.simpleGenericError",
                                          "The same zip code lines have different freight code values. Zip code: " +
                                          postalCode));
          return ae;
        }
        iter.remove();
      } else {
        updateHt.put(postalCode, betV);
      }
  ...
  public static ActionErrors selectAll(HttpServletRequest request,
    ...
    if ("Zip Code".equalsIgnoreCase(sForm.getActConfigType())) {
      ...
      for (int ii = 0; ii < zipCodes.size(); ii++) {
        ...
        selected[ii] = "" + betV.getPostalCode() + "^" + betV.getCity() + "^";
      }
    ...
    if ("County".equalsIgnoreCase(sForm.getActConfigType())) {
      ...
      for (int ii = 0; ii < counties.size(); ii++) {
        ...
        if (betV.getNoModifiyFl()) {
          String ss = (betV.getNoModifiyFl()) ? "0" : "1";
          ss += "^" + betV.getCity() + "^" + betV.getCountyCd() + "^" + betV.getStateProvinceCd() + "^" + betV.getCountryCd() +
            "^";
          selectedAL.add(ss);
        }
      }
  ...
  private static void setSelectedCounties(StoreDistMgrTerrForm sForm) {
    BusEntityTerrViewVector counties = sForm.getCounties();
  ...
  private static void setSelectedZipCodes(StoreDistMgrTerrForm sForm) {
    BusEntityTerrViewVector zipCodes = sForm.getPostalCodes();
    ArrayList selectedV = new ArrayList();
    ArrayList frtTypCdV = new ArrayList();
    for (int ii = 0; ii < zipCodes.size(); ii++) {
      BusEntityTerrView betV = (BusEntityTerrView) zipCodes.get(ii);
      if (betV.getCheckedFl()) {
        String ss = betV.getPostalCode() + "^" + betV.getCity() + "^";
        selectedV.add(ss);
        frtTypCdV.add((betV.getBusEntityTerrFreightCd() == null) ?
                      "" : ss + betV.getBusEntityTerrFreightCd());
      }
    }
    String[] selected = new String[0];
    String[] frtTypCd = new String[0];
