...
//          "LOCATE_ACCOUNT_MULTI_OPT".equals(name) ||
       ...
       }else if ("RECEIVED_DATE_FROM".equals(name) || "RECEIVED_DATE_FROM_OPT".equals(name)) {
            dateSearchReceivedFrom = true;
       }else if ("RECEIVED_DATE_TO".equals(name) || "RECEIVED_DATE_TO_OPT".equals(name)) {
...
<br>
 ...
 <% } // end if ("INVOICE_STATUS_SELECT"...%>
    ...
    <%--START RENDERING "INVOICE_TYPE_2"--%>
    <%
    if ("APPROVED_FLAG".equals(name) || "APPROVED_FLAG_OPT".equals(name)) {
  rendered = true;

    %>
<div class=subheadergeneric>
    <b><app:storeMessage  key="report.text.invoice.status.approvedOnly"/>:</b><br>
	<html:select property="<%=controlEl%>">
...
<br>
    ...
    %>
    <%--END RENDERING "APPROVED_FLAG"--%>
    <%--START RENDERING "SUPPRESS_NOTES"--%>
    <%
  ...
  rendered = true;
    ...
    %>
<div class=subheadergeneric>
    <b><app:storeMessage  key="report.text.invoice.status.suppressNotes"/>:</b><br>
	<html:select property="<%=controlEl%>">
        <html:option value="No">
            <app:storeMessage  key="global.text.no"/>
        </html:option>
...
<DIV class="subheadergeneric">
    <b>
        <app:storeMessage key="report.control.label.RECEIVED_DATE_FROM"/>
		(<%=ClwI18nUtil.getUIDateFormat(request)%>)
    </b>
    <app:dojoInputDate id="receivedDateFrom"
                       name="CUSTOMER_REPORT_FORM"
                       property="<%=controlEl%>"
                       module="clw.CLW"
                       targets="ReveivedDateFrom"/>
     <img id="ReveivedDateFrom" src="../externals/images/showCalendar.gif"
             width=19  height=19 border=0
             onmouseover="window.status='Choose Date';return true"
             onmouseout="window.status='';return true">
</DIV>
...
<DIV class="subheadergeneric">
    <b>
        <app:storeMessage key="report.control.label.RECEIVED_DATE_TO"/>
		(<%=ClwI18nUtil.getUIDateFormat(request)%>)
    </b>
    <app:dojoInputDate id="receivedDateTo"
                       name="CUSTOMER_REPORT_FORM"
                       property="<%=controlEl%>"
                       module="clw.CLW"
                       targets="ReveivedDateTo"/>
     <img id="ReveivedDateTo" src="../externals/images/showCalendar.gif"
             width=19  height=19 border=0
             onmouseover="window.status='Choose Date';return true"
             onmouseout="window.status='';return true">
</DIV>
...
<DIV class="subheadergeneric">
 ...
 %>
	...
	<app:dojoInputDate id="actualBeginDate"
                       ...
                       property="<%=controlEl%>"
                       module="clw.CLW"
                       targets="ActualStartDate"/>
     <img id="ActualStartDate" src="../externals/images/showCalendar.gif"
             width=19  height=19 border=0
             onmouseover="window.status='Choose Date';return true"
             onmouseout="window.status='';return true">
