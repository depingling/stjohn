...
public class SiteMgrNoteLogic {
            ...
            throws Exception
    {
        ...
        String uri = SessionTool.getActualRequestedURI(request);
        int siteId = 0;
        String siteName = "";
        if(uri.indexOf("sitenote")>=0) {
            StoreSiteMgrForm siteForm =
                    (StoreSiteMgrForm) session.getAttribute("STORE_ADMIN_SITE_FORM");
            siteId = siteForm.getIntId();
            siteName = siteForm.getName();
        } else if(uri.indexOf("sitemgrNote")>=0) {
            ...
            throws Exception
    {
        ...
        noteEjb.addNoteTopic(topicName,
                RefCodeNames.PROPERTY_TYPE_CD.SITE_NOTE_TOPIC, userName);
            ...
            throws Exception
    {
        ...
        ArrayList kwAL = parseKeyWords(keyWord);
        NoteViewVector noteVwV =
                noteEjb.getNoteTitles(topicId, siteId, kwAL, pForm.getTextAlsoFl());
        pForm.setNoteList(noteVwV);
        sortByRating(pForm);
