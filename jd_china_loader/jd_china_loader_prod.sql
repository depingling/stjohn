...
PROCEDURE JD_CHINA_USER_LOADER; --- Load JD China User

---New Ones
PROCEDURE JD_CHINA_USER_ACCOUNT_LOADER;
PROCEDURE SITE_CHECK(v_account_id IN NUMBER, v_user_id IN NUMBER);
...
select customer_number,product_sku,current_price from jd_integration.pri_trans_pricing 
where customer_number in(select customer_number from jd_integration.pri_trans_customer
where country = 'CN');

BEGIN
DBMS_OUTPUT.PUT_LINE('Start Pricing Loader');
OPEN c_pri;
LOOP
...
--delete_pricing_item();
DBMS_OUTPUT.PUT_LINE('End Pricing Loader');
END PRICING_LOADER;
...
BEGIN
select count(bus_entity_id) into v_site_id from clw_bus_entity where bus_entity_id in
(select bus_entity_id from clw_property where short_Desc = v_short_site_desc
...
--v_dist_id1 number := 195297;
v_store_id number := 186096;
v_contract_item_id number;
v_short_desc_property VARCHAR2(40) := 'DIST_SITE_REFERENCE_NUMBER';
v_cnt NUMBER := 0;
v_bus_status varchar2(20) := 'ACTIVE';
v_order_guide_id number;
CURSOR c_pr is
select distinct customer_number from jd_integration.pri_trans_pricing;
...
(select a.bus_entity_id,cc.contract_id,c.catalog_id,cog.order_guide_id from 
(select bus_entity_id from clw_bus_entity where bus_entity_id in
(select bus_entity_id from clw_property where short_desc = v_short_desc_property and clw_value = v_customer_number)
and bus_entity_status_cd = v_bus_status and bus_entity_id in(
select bus_entity1_id from clw_bus_entity_assoc where bus_entity2_id in(select bus_entity1_id from clw_bus_entity_assoc where bus_entity2_id = v_store_id))) a,
clw_catalog_assoc cca,clw_contract cc, clw_catalog c,clw_order_guide cog
where a.bus_entity_id = cca.bus_entity_id and cca.catalog_id = c.catalog_id and c.catalog_id = cc.catalog_id
 and cog.catalog_id = c.catalog_id);
...
ELSIF v_cnt = 1 THEN
select a.bus_entity_id,cc.contract_id,c.catalog_id,cog.order_guide_id 
 into v_bus_entity_id,v_contract_id,v_catalog_id,v_order_guide_id from 
(select bus_entity_id from clw_bus_entity where bus_entity_id in
(select bus_entity_id from clw_property where short_desc = v_short_desc_property and clw_value = v_customer_number)
and bus_entity_status_cd = 'ACTIVE' and bus_entity_id in(
select bus_entity1_id from clw_bus_entity_assoc where bus_entity2_id in(select bus_entity1_id from clw_bus_entity_assoc where bus_entity2_id = v_store_id))) a,
clw_catalog_assoc cca,clw_contract cc, clw_catalog c,clw_order_guide cog
where a.bus_entity_id = cca.bus_entity_id and cca.catalog_id = c.catalog_id and c.catalog_id = cc.catalog_id
 and cog.catalog_id = c.catalog_id;
...
DELETE FROM clw_contract_item where contract_id = v_contract_id and item_id not in (select item_id 
from clw_item_mapping where bus_entity_id in(v_dist_id) and item_num in
(select distinct product_sku from jd_integration.pri_trans_pricing
where customer_number = v_customer_number));

DELETE FROM clw_catalog_structure where catalog_id = v_catalog_id and item_id not in (select item_id 
from clw_item_mapping where bus_entity_id in(v_dist_id) and item_num in
(select distinct product_sku from jd_integration.pri_trans_pricing
where customer_number = v_customer_number));

DELETE FROM clw_order_guide_structure where order_guide_id = v_order_guide_id
...
v_uom varchar2(20);
v_count number;
v_bus_entity_id_reg number := 186098;
...
v_is_haz varchar2(10);

CURSOR c_item is

select distinct TI.ITEM_NUMBER2,TI.description,TI.uom_primary_pricing,TI.is_item_hazardous
from jd_integration.pri_trans_item TI 
where TI.connection_status in('unprocessed','updated')  and TI.item_number2 in
(select item_number_2 from jd_integration.PRI_ITEM_MASTER where item_master_id in
(select item_master_id from jd_integration.pri_item_branch where business_unit in(
select business_unit from jd_integration.JD_BUS_UNIT_COUNTRY_CD where country_code = 'CN')));
--select ITEM_NUMBER2,description,uom_primary_pricing  from temp_item_loader;
BEGIN

DBMS_OUTPUT.PUT_LINE('Start Item Loader');
OPEN c_item;
LOOP
FETCH c_item into v_item_num,v_description,v_uom,v_is_haz;
EXIT WHEN c_item%NOTFOUND;

IF v_is_haz = 'Yes' THEN
--v_dist_id := v_bus_entity_id_haz; --changed on 02042010
v_dist_id := v_bus_entity_id_reg;
ELSE
v_dist_id := v_bus_entity_id_reg;
END IF;

DBMS_OUTPUT.PUT_LINE('v_item_num = ' || v_item_num);
select count(*) into v_count from clw_item_mapping where item_num = v_item_num 
and status_cd = v_item_status_cd and bus_entity_id in (v_bus_entity_id_reg);

--and status_cd = v_item_status_cd and bus_entity_id in (v_bus_entity_id_reg,v_bus_entity_id_haz);--02042010


IF v_count > 1 THEN
DBMS_OUTPUT.PUT_LINE('Error Duplicate Item Found :');
DBMS_OUTPUT.PUT_LINE('select item_id from clw_item_mapping');
DBMS_OUTPUT.PUT_LINE('where item_num ='|| v_item_num || 'and bus_entity_id ='|| v_bus_entity_id_reg);

ELSIF v_count = 1 THEN
DBMS_OUTPUT.PUT_LINE('Old Item');
DBMS_OUTPUT.PUT_LINE('Dist Id : ' || v_dist_id);

--select item_id into v_item_id from clw_item_mapping where item_num = v_item_num and bus_entity_id in(v_bus_entity_id_reg,v_bus_entity_id_haz);
select item_id into v_item_id from clw_item_mapping where item_num = v_item_num and bus_entity_id in(v_bus_entity_id_reg);
...
EXECUTE IMMEDIATE 'update clw_item_mapping set bus_entity_id = :1,mod_date = sysdate,mod_by = :2 where item_id = :3 and bus_entity_id in(:4)'
					USING v_dist_id,v_mod_by,v_item_id,v_bus_entity_id_reg;
				   
--EXECUTE IMMEDIATE 'update clw_item_mapping set bus_entity_id = :1,mod_date = sysdate,mod_by = :2 where item_id = :3 and bus_entity_id in(:4,:5)'
--					USING v_dist_id,v_mod_by,v_item_id,v_bus_entity_id_reg,v_bus_entity_id_haz;
					
					

EXECUTE IMMEDIATE 'update clw_catalog_structure set bus_entity_id = :1,mod_date = sysdate,mod_by = :2
					where item_id = :3 and bus_entity_id in(:4)'
					USING v_dist_id,v_mod_by,v_item_id,v_bus_entity_id_reg;
					
--EXECUTE IMMEDIATE 'update clw_catalog_structure set bus_entity_id = :1,mod_date = sysdate,mod_by = :2
--					where item_id = :3 and bus_entity_id in(:4,:5)'
--					USING v_dist_id,v_mod_by,v_item_id,v_bus_entity_id_reg,v_bus_entity_id_haz;
...
values(:1,:2,:3,:4,:5,sysdate,:6,sysdate,:7)'
...
--update clw_item_mapping set bus_entity_id = v_bus_entity_id_haz,mod_date = sysdate,
--mod_by = v_mod_by where item_num in
--(select distinct TI.ITEM_NUMBER2 from jd_integration.pri_trans_item TI 
--where TI.is_item_hazardous = 'Yes' and TI.connection_status in('unprocessed','updated') 
--and TI.item_number2 in(select item_number_2 from jd_integration.PRI_ITEM_MASTER 
--where item_master_id in
--(select item_master_id from jd_integration.pri_item_branch where business_unit in(
--select business_unit from jd_integration.JD_BUS_UNIT_COUNTRY_CD where country_code = 'CN'))))
--and bus_entity_id = v_bus_entity_id_reg;

update jd_integration.pri_trans_item set connection_status = 'processed',
modified_by = 'Manoj',modified_date=sysdate 
where connection_status in('unprocessed','updated')  and item_number2 in
(select item_number_2 from jd_integration.PRI_ITEM_MASTER where item_master_id in
(select item_master_id from jd_integration.pri_item_branch where business_unit in(
select business_unit from jd_integration.JD_BUS_UNIT_COUNTRY_CD where country_code = 'CN')));
...
DBMS_OUTPUT.PUT_LINE('End Item Loader');
END ITEM_LOADER;
...
v_customer_number varchar2(40);
v_parent_customer_number varchar2(40);
...
from jd_integration.pri_trans_customer where customer_type = 'ACCOUNT' 
and country in('CN','HK') and connection_status in('unprocessed','updated');


cursor c_acc_site is
select customer_name,customer_number,parent_customer_number,contact_name,address_line_1,address_line_2,address_line_3,address_line_4,
city,state,country,postal_code,phone_number,fax_number
from jd_integration.pri_trans_customer where customer_type = 'ACCOUNT_AND_SITE' 
and country in('CN','HK') and connection_status in('unprocessed','updated');
...
select customer_name,customer_number,parent_customer_number,contact_name,address_line_1,address_line_2,address_line_3,address_line_4,
city,state,country,postal_code,phone_number,fax_number
from jd_integration.pri_trans_customer where customer_type = 'SITE' and country in('CN','HK')
...
BEGIN
DBMS_OUTPUT.PUT_LINE('Start Customer Loader ACCOUNT_AND_SITE');
...
END IF;
--DBMS_OUTPUT.PUT_LINE('End ACCOUNT_AND_SITE Account Loader');
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------
--DBMS_OUTPUT.PUT_LINE('Start ACCOUNT_AND_SITE Site Loader');
select count(*) into v_c_site_up from clw_bus_entity where bus_entity_id in
...
select bus_entity_id into v_c_site_up from clw_bus_entity where bus_entity_id in
(select bus_entity1_id from clw_bus_entity_assoc where bus_entity1_id in
(select bus_entity_id from clw_property where short_desc = v_property_short_desc_s 
and clw_value = v_customer_number) and bus_entity2_id in
(select bus_entity1_id from clw_bus_entity_assoc where bus_entity2_id = v_bus_entity_id_store))
...
update jd_integration.pri_trans_customer set connection_status = 'processed' 
where customer_type = 'ACCOUNT_AND_SITE' 
and country in('CN','HK') and connection_status in('unprocessed','updated');

DBMS_OUTPUT.PUT_LINE('END Load Accounts_And_Sites');
...
update jd_integration.pri_trans_customer set connection_status = 'processed' 
where customer_type = 'ACCOUNT' and country in('CN','HK')
and connection_status in('unprocessed','updated');
...
DBMS_OUTPUT.PUT_LINE('Load Sites Customer_Number:' || v_customer_number);
...
--DBMS_OUTPUT.PUT_LINE('Load Sites v_c_site_up: v_customer_number' || v_c_site_up || ':' ||v_customer_number);
...
ELSIF v_c_site_up = 1 THEN
--IF v_c_site_up >= 1 THEN
DBMS_OUTPUT.PUT_LINE('Old Site v_customer_number' ||v_customer_number);
--DBMS_OUTPUT.PUT_LINE('OLD Load Sites v_customer_number' ||v_customer_number);
select bus_entity_id into v_c_site_up from clw_bus_entity where bus_entity_id in
(select bus_entity1_id from clw_bus_entity_assoc where bus_entity1_id in
(select bus_entity_id from clw_property where short_desc = v_property_short_desc_s 
and clw_value = v_customer_number) and bus_entity2_id in
...
ELSIF v_c_site_up = 0 THEN
DBMS_OUTPUT.PUT_LINE('New Site');
...
--DBMS_OUTPUT.PUT_LINE('v_bus_entity_id_s = '|| v_bus_entity_id_s );
...
select clw_bus_entity_assoc_seq.nextval into v_bus_entity_assoc_id from dual;
--DBMS_OUTPUT.PUT_LINE('v_bus_entity_assoc_id = '|| v_bus_entity_assoc_id );
...
--DBMS_OUTPUT.PUT_LINE('v_bus_entity_id_a = '|| v_bus_entity_id_a );
...
--DBMS_OUTPUT.PUT_LINE('v_phone_id = '|| v_phone_id );
...
--craete catalog and aasociate it with the new site
select clw_catalog_seq.nextval into v_catalog_id from dual;
--DBMS_OUTPUT.PUT_LINE('v_catalog_id = '|| v_catalog_id );
...
select clw_catalog_assoc_seq.nextval into v_catalog_assoc_id from dual;
--DBMS_OUTPUT.PUT_LINE('v_catalog_assoc_id = '|| v_catalog_assoc_id );
...
select clw_catalog_assoc_seq.nextval into v_catalog_assoc_id from dual;
--DBMS_OUTPUT.PUT_LINE('v_catalog_assoc_id 2 = '|| v_catalog_assoc_id );
...
--DBMS_OUTPUT.PUT_LINE('v_bus_entity_id_ac = '|| v_bus_entity_id_ac );
...
--DBMS_OUTPUT.PUT_LINE('Enddddd v_catalog_assoc_id = '|| v_catalog_assoc_id );
ELSE
...
update jd_integration.pri_trans_customer set connection_status = 'processed'
where customer_type = 'SITE' and country in('CN','HK')
and connection_status in('unprocessed','updated');
...
commit;
DBMS_OUTPUT.PUT_LINE('End Customer Loader');
END CUSTOMER_LOADER;
--********************************************************************************************************************
---END Customer Loader
--********************************************************************************************************************
...
--Start User Loader
--******************************************************************************************************************
PROCEDURE JD_CHINA_USER_LOADER IS
v_user_id NUMBER;
v_first_name VARCHAR2(80);
v_last_name VARCHAR2(80);
v_user_name VARCHAR2(80);
v_password VARCHAR2(80) := 'cfcd208495d565ef66e7dff9f98764da';
v_user_status_cd VARCHAR2(20) := 'ACTIVE';
--v_user_role_cd VARCHAR2(80) := 'SP^CI^OA^OPmt^CC^PR^eST^eBT^';
--v_user_role_cd VARCHAR2(80) := 'SP^CI^OA^';
v_user_role_cd VARCHAR2(80) := 'SP^BO^CI^OA^';
...
CURSOR c_user is 
SELECT DISTINCT SALES_REP_CODE,sales_rep_name
FROM jd_integration.PRI_TRANS_SALES_REP where customer_number in
(select customer_number from jd_integration.pri_trans_customer where country in('CN','HK')) 
and connection_status = 'unprocessed';
--and sales_rep_code = '9796228';

CURSOR c_user_assoc is 
select sales_rep_code,customer_number from jd_integration.pri_trans_sales_rep
where customer_number in
(select customer_number from jd_integration.pri_trans_customer where country in('CN','HK'))
and customer_type in('ACCOUNT_AND_SITE','SITE')
and sales_rep_code = v_user_name;

BEGIN
OPEN c_user;
LOOP
FETCH c_user INTO v_user_name,v_first_name;
...
--v_last_name := v_user_name|| '-' || v_user_id;
...
CLOSE c_user;
COMMIT;
END JD_CHINA_USER_LOADER;
--******************************************************************************************************************
--End User Loader
--******************************************************************************************************************


---**********************Start JD CHina USER ACCCOUNT LOADER

PROCEDURE JD_CHINA_USER_ACCOUNT_LOADER IS
v_user_id NUMBER;
v_first_name VARCHAR2(80);
v_last_name VARCHAR2(80);
v_user_name VARCHAR2(80);
...
CURSOR c_user_assoc_acc is 
select sales_rep_code,customer_number from jd_integration.pri_trans_sales_rep
 where customer_number in
(select customer_number from jd_integration.pri_trans_customer where country in('CN','HK')) and customer_type = 'ACCOUNT'
 and sales_rep_code = v_user_name;
...
select user_id into v_user_id from clw_user where user_name = v_user_name;
DBMS_OUTPUT.PUT_LINE('USER_ID = ' || v_user_id || ' user_name = ' || v_user_name);
OPEN  c_user_assoc_acc;

LOOP

FETCH c_user_assoc_acc into v_sales_rep_code,v_customer_number;
EXIT WHEN c_user_assoc_acc%NOTFOUND;

select count(*) into v_account_id from clw_bus_entity where bus_entity_id in
(select bus_entity1_id from clw_bus_entity_assoc where bus_entity1_id in
(select bus_entity_id from clw_property where short_desc = v_property_short_desc_a 
and clw_value = v_customer_number) and bus_entity2_id in(v_bus_entity_id_store))
and bus_entity_status_Cd = v_bus_entity_status_cd;

IF v_account_id > 1 THEN
DBMS_OUTPUT.PUT_LINE('Error Line found same site for more then 1 account');
DBMS_OUTPUT.PUT_LINE('select * into v_account_id from clw_bus_entity where bus_entity_id in');
DBMS_OUTPUT.PUT_LINE('(select bus_entity1_id from clw_bus_entity_assoc where bus_entity1_id in');
DBMS_OUTPUT.PUT_LINE('(select bus_entity_id from clw_property where short_desc = ' || v_property_short_desc_a );
DBMS_OUTPUT.PUT_LINE('and clw_value = '|| v_customer_number || ') and bus_entity2_id in( ' || v_bus_entity_id_store || '))');
DBMS_OUTPUT.PUT_LINE('and bus_entity_status_Cd = ' || v_bus_entity_status_cd);
...
select count(*) into v_user_assoc_id
from clw_user_assoc where user_id = v_user_id and bus_entity_id = v_account_id;

if v_user_assoc_id > 1 THEN
DBMS_OUTPUT.PUT_LINE('Error Duplicate entry found in user assoc table for');
DBMS_OUTPUT.PUT_LINE('bus_entity_id = '|| v_account_id || ' user_id = ' || v_user_id);

ELSIF v_user_assoc_id = 1 THEN
DBMS_OUTPUT.PUT_LINE('User ' || v_user_name || ' is already associated to Account_id ' || v_account_id);

--check all the sites within this account
DBMS_OUTPUT.PUT_LINE('Start Checking Sites');
JD_CHINA_LOADER.SITE_CHECK(v_account_id,v_user_id);
DBMS_OUTPUT.PUT_LINE('END Checking Sites');

ELSE

---Associate Account and all site within that account
SELECT clw_user_assoc_seq.nextval into v_user_assoc_id from dual;
...
ELSE
DBMS_OUTPUT.PUT_LINE('Account Does Not Exists customer_number = ' || v_customer_number);

END IF;
END LOOP;
COMMIT;
CLOSE c_user_assoc_acc;
...
update jd_integration.PRI_TRANS_SALES_REP set connection_status = 'processed' where SALES_REP_CODE = v_user_name 
and sales_rep_name = v_first_name and connection_status = 'unprocessed';

END LOOP;
COMMIT; ---------check this
CLOSE c_user;

END JD_CHINA_USER_ACCOUNT_LOADER;


PROCEDURE SITE_CHECK(v_account_id IN NUMBER, v_user_id IN NUMBER) IS
v_site_id number;
v_user_assoc_id number;
v_user_assoc_cd_s VARCHAR2(40) := 'SITE';
v_add_by VARCHAR2(20) := 'Manoj';
...
CURSOR c_user_assoc_acc_site is
select distinct bus_entity1_id from clw_bus_entity_assoc where bus_entity2_id = v_account_id;

BEGIN
open c_user_assoc_acc_site;
LOOP
fetch c_user_assoc_acc_site into v_site_id;
exit when c_user_assoc_acc_site%notfound;

select count(*) into v_user_assoc_id from clw_user_assoc where user_id = v_user_id and bus_entity_id = v_site_id;
...
ELSIF v_user_assoc_id = 1 THEN
DBMS_OUTPUT.PUT_LINE('User ID' || v_user_id || ' is already associated to site_id ' || v_site_id);
...
--END JD China USER ACCOUNT LOADER 
--***********************************************************************
--commit;
END JD_CHINA_LOADER;
/
