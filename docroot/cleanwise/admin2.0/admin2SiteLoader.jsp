...
<bean:define id="theForm" name="ADMIN2_SITE_LOADER_MGR_FORM" type="com.cleanwise.view.forms.Admin2SiteLoaderMgrForm"/>
<%
...
<div class="text">
  ...
  <html:form styleId="1016"  action="/admin2.0/siteloader.do"
            scope="session">
...
<tr>
    ...
    <td width="170"><html:submit property="action" value="Export Template" styleClass="submitButton150"  /></td>
    <td>
        <% if (theForm.getDownloadErrorButton()) { %>
        <html:submit property="action" value="Download Errors" styleClass="submitButton150"  />
        <% } else if (theForm.getSuccessfullyLoaded()) { %>
        <html:submit property="action" value="Export Results" styleClass="submitButton150"  />
        <% } %>
    </td>
</tr>
...
<table ID=1010 cellspacing="0" border="0" width="769" class="mainbody">
    ...
    <td colspan='4'><b>Select File:</b>
        <html:file name="ADMIN2_SITE_LOADER_MGR_FORM" property="xlsFile" size='80'/>
...
%>
                <p class=rederrorbig><app:storeMessage key="accAdmin.loaders.line"/> <%=j+1%>, <%=rowErrors[i]%></p>
<%          }
        }
    }
%>
...
<jsp:include flush='true' page="locateStoreAccount.jsp">
   <jsp:param name="jspFormAction" 	value="/admin2.0/siteloader.do" />
   <jsp:param name="jspFormName" 	value="ADMIN2_SITE_LOADER_MGR_FORM" />
