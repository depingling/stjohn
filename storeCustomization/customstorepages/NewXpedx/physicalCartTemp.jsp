...
<table>
<tr>
	<%--This is assumed, but double check for sure --%>
	<%if (ShopTool.isPhysicalCartAvailable(request)) {%>
	    ...
	    if(invPeriod != null && invPeriod.getEndDate() != null){
	    	endDate = ClwI18nUtil.formatDate(request,invPeriod.getEndDate(),DateFormat.MEDIUM);
	    }%>﻿
		<td style="font-weight:bold; color:red"><app:storeMessage key="shoppingCart.text.physicalInventoryText" arg0="<%=endDate%>"/><br></td>
	<%}else{ %>
...
<%--
<html:form action="/store/physicalCart.do">
--%>
<table cellpadding="1" cellspacing="0" align="center"   width="<%=Constants.TABLEWIDTH800%>" >
  ...
  <tr><td>
                ...
                <jsp:include flush='true' page='<%=ClwCustomizer.getStoreFilePath(request,"f_shoppingPhysItems.jsp")%>'>
