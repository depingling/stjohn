...
function viewPrinterFriendly(oid) {
  var loc = "printerFriendlyOrder.do?action=view&orderId=" + oid + "&showSize=n&showMfg=n&showMfgSku=n;
...
boolean disableButtonsForApprover=true;
...
<% } else {
  ...
  if(canApproveIdV.contains(opIdI)) {
  	disableButtonsForApprover = false;
%>
...
<% String myOrderStatus=com.cleanwise.view.utils.ShopTool.xlateStatus(theOrder);
//OrderStatusDescData myOrderStatusDetail = theForm.getOrderStatusDetail();
//boolean myShipFlag = false;
//myShipFlag = myOrderStatusDetail.isShipped();
if( myOrderStatus.equals("Shipped")) { %>
  <td>Complete
...
<% if(!toConsolidateOrderFl && !disableButtonsForApprover) { %>
...
<% } %>
<% if(!consolidatedOrderFl && !disableButtonsForApprover) { %>
