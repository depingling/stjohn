...
<logic:notEmpty name="esw.ControlsForm" property="siteShoppingControlForm.shoppingRestrictionsViews"><%if (canEditProductLimits) {%>
							<a class="blueBtnLargeExt" onclick="javascript:setFieldsAndSubmitForm('controlsForm', 'operationId', '<%=Constants.PARAMETER_OPERATION_VALUE_UPDATE_PRODUCT_LIMITS%>');"><span><app:storeMessage key="global.action.label.update" /></span></a> <hr /><%}%>
			                <p class="required right"><app:storeMessage key="userportal.esw.text.resetToAcctUnlimited" /></p>
							<table class="order">
                                <thead>
                                	<tr>
...
String itemIdMaxAllowedInp = "itemIdMaxAllowed(" + itemId + ")";
String itemIdRestrictionDaysInp = "itemIdRestrictionDays(" + itemId + ")";   
...
%>
                                    <tr>
										...
										<td class="right">										
                                            <div class="qtyInput">
                                                <html:text name="esw.ControlsForm" property="<%=itemIdMaxAllowedInp%>" value="<%=siteQtyToDiplay%>" size='3'/>  
											</div>
										</td>
										<td class="right">										
                                            <div class="qtyInput">
                                                <html:text name="esw.ControlsForm" property="<%=itemIdRestrictionDaysInp%>" value="<%=restrictionDays%>" size='3'/>  
											</div>
										</td>
...
function submitSortBy(sortField) {
    ...
    $('#operationId').val('<%=Constants.PARAMETER_OPERATION_VALUE_EDIT_PRODUCT_LIMITS%>');
	$("#controlsForm").submit();
	return false;
}
</script>
