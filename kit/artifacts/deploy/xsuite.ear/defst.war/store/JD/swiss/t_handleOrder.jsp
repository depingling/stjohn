...
<%
 ...
 ClwI18nUtil clwI18n = ClwI18nUtil.getInstance(request, orderLocale, -1);
  ...
  SimpleDateFormat sdfMod = new SimpleDateFormat("yyyy-MM-dd HH:mm");
  sdfMod.setTimeZone(timeZone);
    ...
    <td align="left" class="text"><b>
      ...
      %>
        ...
        <app:storeMessage key="shop.orderStatus.text.ModificationStartedOnBy"
          arg0='<%=sdfMod.format(omD.getModDate())%>'
          arg1='<%=omD.getValue()%>'/>
        </font>
      <% } %>
     <% } %>
...
<table align=center CELLSPACING=0 CELLPADDING=0 width="<%=Constants.TABLEWIDTH%>">  <%/* Detail Information Table   */%>
  <tr>
    ...
    font-weight: bold;">
     ...
     <table align=center CELLSPACING=0 CELLPADDING=0 >
       ...
       <tr>
         ...
         Date date = theOrder.getOrderDate();
         Date time = theOrder.getOrderTime();
  ...
  if(pendingDate!=null && pendingDate.length()>7) {
	...
	//first convert to US date pattern and then to locale pattern because this date should always be stored in US format in the db
	SimpleDateFormat defaultFormat = new SimpleDateFormat("MM/dd/yyyy");
	Date defaultDate = defaultFormat.parse(pendingDate);
	
	String datePattern = ClwI18nUtil.getDatePattern(request);	
	SimpleDateFormat psdf = new SimpleDateFormat(datePattern);
	
	Date pDate = psdf.parse(psdf.format(defaultDate));
	formattedPendingDate = ClwI18nUtil.formatDateInp(request, pDate, timeZone.getID() );
  }
  %>
    ...
    <tr><td colspan="3" class="text"><b><app:storeMessage key="shop.orderStatus.text.notes:"/></b></td><tr>
      ...
      <tr><td colspan="3">
        ...
        <app:storeMessage key="<%=messKey%>"/>
          <%if(opD.getArg0() != null){
          Object args[] = new Object[] { opD.getArg1(), opD.getArg2(), opD.getArg3() };
          String mess = ClwI18nUtil.getMessage(request,opD.getArg0(),args,true);
          if(mess==null){
            mess = opD.getArg0();
          }
          %>
          ...
          <% if(opD.getApproveDate()!=null) {
				Date approveOnDate = opD.getApproveDate();
				String formattedApproveOnDate = ClwI18nUtil.formatDateInp(request, approveOnDate, timeZone.getID() );
		  %>
          (<app:storeMessage key="shop.orderStatus.text.clearedOnBy"
            arg0='<%=formattedApproveOnDate%>'
            arg1='<%=opD.getModBy()%>'/>)
            <% } else {
   ...
   <logic:iterate name="customerOrderNotes"id="note" type="com.cleanwise.service.api.value.OrderPropertyData">
    <tr>
      ...
      <logic:present name="note" property="addDate">
        ...
        <b>
		<%
		String formattedCommentDate = ClwI18nUtil.formatDateInp(request, commentDate, timeZone.getID() );
		%>
...
<!-- cartDiscount -->
<% String stringDiscountAmt = theOrder.getMetaValue(RefCodeNames.CHARGE_CD.DISCOUNT); %>
<%  BigDecimal v_discountAmt = new BigDecimal(0); %>
<% if ( stringDiscountAmt != null ) {
	v_discountAmt = new BigDecimal(stringDiscountAmt);
   }
   v_discountAmt = v_discountAmt.setScale(2, BigDecimal.ROUND_HALF_UP); 
   if (v_discountAmt.compareTo(new BigDecimal(0))!=0 ) { 
	  if (v_discountAmt.compareTo(new BigDecimal(0)) > 0 ) {   // always convert Discout to negative value, if it is positive	   
			  v_discountAmt = v_discountAmt.negate(); 
	  } 
%>
      <tr>
      <td class="smalltext"><b><html>Discount:</html></b></td>
      <td  align=right>
      <%=clwI18n.priceFormat(v_discountAmt,"&nbsp;")%>
      </td>
      </tr>
<% } %>
<!-- Discount: End  -->
...
<%    totalOrderPrice = theOrder.getSumOfAllOrderCharges().add(v_discountAmt); %>
<%=clwI18n.priceFormat(totalOrderPrice,"&nbsp;")%>
<% } else {  %>
*
<% } %>
</td>
...
<% } %>
</table>   <%/*End : Detail Information Table   */%>
