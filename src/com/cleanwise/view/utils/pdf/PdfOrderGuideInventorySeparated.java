 ...
 * @author  bstevens
 */
public class PdfOrderGuideInventorySeparated extends PdfOrderGuide {
  ...
  public PdfOrderGuideInventorySeparated() {
      super();
  }

  public PdfOrderGuideInventorySeparated(HttpServletRequest pRequest, String pCatalogLocaleCd) {
    super();
 ...
 public void init (HttpServletRequest pRequest, String pCatalogLocaleCd) {
  ...
  private Table makeItemElement(ShoppingCartItemData pItm) throws DocumentException {
    ...
    Table itmTbl = new Table(mColumnCount);
    itmTbl.setWidth(100);
    itmTbl.setPadding(1);
    itmTbl.setBorderWidth(0.5f);
    itmTbl.setOffset(0);
  ...
  public void generatePdf(UserShopForm sForm, List pItems, StoreData pStore,
    ...
    try {
      ...
      ShoppingCartItemDataVector pInvItemsDV = new ShoppingCartItemDataVector();
...
//    sort the inventory items by category and sku num
      ShoppingCartItemDataVector retItemDV = new ShoppingCartItemDataVector();
      if(!pInvItemsDV.isEmpty()){
          int size=pInvItemsDV.size();
          ShoppingCartItemData[] items = new ShoppingCartItemData[size];
          for(int ii=0; ii<size; ii++) {
                  items[ii] = (ShoppingCartItemData) pInvItemsDV.get(ii);
          }
          for(int i=0; i<size-1; i++){
                  ...
                  for (int j = 1; j < size-i; j++) {
                          ...
                          ShoppingCartItemData item1 = items[j-1];
                          ShoppingCartItemData item2 = items[j];
      ...
      if(modernShoppingFl) {
        if (invItems) {
            ...
            drawHeader(document, pageNumber,
                       ...
                       pImageName, true);
            String invItemsStr =
                    ClwI18nUtil.getMessage(mRequest, "shop.og.table.header.title.inventory", null) + " ";

            document.add(makePhrase(invItemsStr.toUpperCase(),smallHeading, true));

            String prevCat = null;

            //for (int i = 0; i < pItems.size(); i++) {
            for(int i=0; i<retItemDV.size(); i++){

              //ShoppingCartItemData sci = (ShoppingCartItemData) pItems.get(i);
              ShoppingCartItemData sci = (ShoppingCartItemData) retItemDV.get(i);
              ...
              if (writer.fitsPage(itmTable, document.bottomMargin() + 10)) {
                if (sForm.getOrderBy() == Constants.ORDER_BY_CATEGORY) {
                  if (null == prevCat ||
                      !prevCat.equals(sci.getCategoryPath())
                    ) {
        ...
        if (nonInvItems) {
            ...
            String invItemsStr =
                    ClwI18nUtil.getMessage(mRequest, "shop.og.table.header.title.regular", null) + " ";
            ...
            for (int i = 0; i < pItems.size(); i++) {
              ShoppingCartItemData sci = (ShoppingCartItemData) pItems.get(i);
              if (sci.getIsaInventoryItem() == true) {
                continue;
              }
              Table itmTable = makeItemElement(sci);
