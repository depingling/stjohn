...
<%
 ...
 if(ccNumber!=null && ccNumber.trim().length()>0) {
  ...
  boolean invShopFl = shoppingCart.getSite().hasInventoryShoppingOn();
  boolean modernShopFl = shoppingCart.getSite().hasModernInventoryShopping();
...
<app:commonDefineProdColumnCount
    ...
    />
<%int colCount = ((Integer)pageContext.getAttribute("colCountInt")).intValue() -1;%>
         ...
         <table cellspacing=0 cellpadding=0 valign=top width="100%">
           <tr>
             <td valign=top>
               ...
               <table valign=top width="100%" border="0" cellpadding="1" cellspacing="0">
                 <tr>
                   ...
                   <td class="text" align="left" width="40%">
                   ...
                   <td class="text" align="left" width="30%"><% if(theForm.getRequestedShipDate()!=null && theForm.getRequestedShipDate().trim().length()>0) {%>
	                     <b><app:storeMessage key="Jd.swiss.shop.checkout.text.requestedDeliveryDate:"/></b>
                			&nbsp;<br><%=ClwI18nUtil.formatDate(request,theForm.getRequestedShipDate(),DateFormat.FULL)%>
	                     <% } else {%>
                     		&nbsp;
                     	 <% } %>
                   </td>
...
%>
<tr><td colspan="<%=colCount%>">
      <jsp:include flush='true' page='<%=ClwCustomizer.getStoreFilePath(request,"f_confirmRushOldShoppingItems.jsp")%>'>
        <jsp:param name="allowEdits" value="false"/>
        <jsp:param name="showSelectCol" value="true"/>
        <jsp:param name="view" value="checkout"/>
        <jsp:param name="orderBy" value="<%=theForm.getSortBy()%>"/>
...
%>
  <tr>
  <td colspan="<%=colCount + 1%>" class="tableoutline"><img src="/images/cw_spacer.gif" height="1"></td>
  </tr>


  <tr>
  <td class="text" align="right" colspan="<%=(colCount)%>">
  	<div><b><app:storeMessage key="shoppingItems.text.productTotal:"/></b></div>
  </td>
  ...
  <table border="0" align="center" cellspacing="0" cellpadding="0" width="90%">
     ...
     <table border="0" cellspacing="0" cellpadding="0" align="center">
        ...
        <tr><td class="smalltext"><b><app:storeMessage key="shop.checkout.text.subtotal"/></b></td>
         ...
         <!-- cartDiscount -->
         <% BigDecimal discountAmt = theForm.getDiscountAmt();// Getting the discount %>
...
--%>
        ...
        %>
        <tr> <td class="smalltext"><b><app:storeMessage key="shop.checkout.text.totalExcludingVOC"/></b></td>
        <% BigDecimal lTotal = theForm.getCartAmt().add(theForm.getFreightAmt());
           ...
           lTotal = lTotal.add(theForm.getDiscountAmt()); // SVC: new stmt=> Discount
//           lTotal = lTotal.add(rushCharge);
  ...
  <%if(!usedCreditCard){%><%-- find me on account bill to --%>
      ...
      <% }%>
      <b><app:storeMessage key="shop.checkout.text.zipCity:"/></b>&nbsp;
