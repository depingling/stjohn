...
public class OutboundXpedxOrderDashboardCXMLPurchaseOrderCleanwiseFull extends InterchangeOutboundSuper implements OutboundTransaction{
	...
	public OutboundXpedxOrderDashboardCXMLPurchaseOrderCleanwiseFull(){
	...
	throws Exception {
		...
		createElementIfTextNotEmpty(orderRequestHeader, "Comments",
				Utility.toXMLString(currOrder.getComments()));
		...
		for (int i = 0; i < items.size(); i++) {
			...
			String distItemNum = currItem.getDistItemSkuNum();
			if(distItemNum != null){
				distItemNum = distItemNum.trim();
			}
			createElement(createElement(itemOut, "ItemID"), "SupplierPartID",distItemNum);

			final Element itemDetail = createElement(itemOut, "ItemDetail");
			createElement(createElement(itemDetail, "UnitPrice"), "Money",
					currItem.getDistItemCost().toString(),
					new String[][] { { "currency", currOrder.getCurrencyCd() } });
			// TODO Unintelligible: How get 'Description'.
