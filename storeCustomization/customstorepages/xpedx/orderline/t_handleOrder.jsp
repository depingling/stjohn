...
function viewPrinterFriendly(oid) {
  var loc = "printerFriendlyOrder.do?action=view&orderId=" + oid + "&showMfg=n&showMfgSku=n&showPar=n";
...
<% for(int ii=0; ii<opDV.size(); ii++) {
  ...
  if(messKey==null) {
      message = opD.getValue();
  } else {
  String argType0 = opD.getArg0TypeCd();
  if("MESSAGE_KEY".equals(argType0)) {
    String[] args = new String[3];
    args[0] = opD.getArg1();
    args[1] = opD.getArg2();
    args[2] = opD.getArg3();
    String ruleMess = ClwI18nUtil.getMessage(request,opD.getArg0(),args);
    args[0] = ruleMess;
    message = ClwI18nUtil.getMessage(request,messKey,args);
  } else {
...
%>
  <tr>
    <td colspan="3" class="text"><b><app:storeMessage key="label.customer.order.notes" arg0='<%=theOrder.getBillShortDesc()%>' />:</b><br>
     <TEXTAREA name='customerComment' rows="3" cols="55"></TEXTAREA>
   <html:button styleClass="store_fb" property="action"
    onclick="actionMultiSubmit('BBBBBBB', 'addComment');">
...
<td class="shopcharthead"><div class="fivemargin"><app:storeMessage key="shoppingItems.text.lineTotal"/></div></td>
</logic:equal>
...
<logic:equal name="<%=Constants.APP_USER%>" property="userStore.storeType.value" value="<%=RefCodeNames.STORE_TYPE_CD.DISTRIBUTOR%>">
    <logic:present name="order">
        ...
        <logic:iterate id="item" name="order" property="orderJoinItems"
            ...
            <%
                if(((OrderItemData)item.getOrderItem()).getDistErpNum()!= null
                        && ((OrderItemJoinData)item).getOrderFreight()!=null
                        &&!distErpSet.contains(((OrderItemData)item.getOrderItem()).getDistErpNum())){

                    distErpSet.add(((OrderItemData)item.getOrderItem()).getDistErpNum());
            %>
            <tr>
...
<logic:equal name="item" property="isAnInventoryItem" value="true">
  <td>
  <%--<span class="inv_item">--%>
  <bean:write name="item" property="shortDesc"/>
  <%--</span>--%>
  </td>
</logic:equal>
<logic:equal name="item" property="isAnInventoryItem" value="false">
...
<td><bean:write name="orderItemData" property="itemSize"/></td>
<td><bean:write name="orderItemData" property="itemPack"/></td>
<td><bean:write name="orderItemData" property="itemUom"/></td>
<td>&nbsp;</td>
<logic:equal name="<%=Constants.APP_USER%>" property="showPrice" value="true">
<td align="right">
...
<logic:present name="item" property="amount">
<bean:define id="l_amt" type="java.math.BigDecimal"name="item" property="amount"/>
<%l_amt=l_amt.multiply(new BigDecimal(item.getQty()));%>
<%=clwI18n.priceFormat(l_amt,"&nbsp;")%>
</logic:present>
</td>
<%--end extended amount--%>
...
<logic:present name="order" property="orderData.totalTaxCost">
<bean:define id="v_tax" type="java.math.BigDecimal"
  name="order" property="orderData.totalTaxCost"/>
<% if(v_tax != null && v_tax.compareTo(new BigDecimal(0)) != 0) { %>
<tr>
<td class="smalltext"><b><app:storeMessage key="shop.orderStatus.text.tax:"/></b></td>
<td  align=right>
<% if(!toConsolidateOrderFl) { %>
<%=clwI18n.priceFormat(v_tax,"&nbsp;")%>
<% } else {  %>
*
<% } %>
</td>
...
<html:button styleClass="store_fb" property="action"
  onclick="actionMultiSubmit('BBBBBBB', 'reject');">
...
<% if(!consolidatedOrderFl) { %>
<html:button styleClass="store_fb" property="action"
  onclick="actionMultiSubmit('BBBBBBB', 'modify');">
...
%>
<jsp:include page="<%=ClwCustomizer.getRootFilePath(request, \"replacedOrders.jsp\")%>" flush="true" />
