...
<%
	...
	boolean isShowInactiveFlag = false;
...
<bean:define id="theForm" name="CUSTOMER_REPORT_FORM" type="com.cleanwise.view.forms.CustAcctMgtReportForm"/>
    ...
    <jsp:param name="noShowRefNumField" value="true"/>
...
<jsp:include flush='true' page="/reporting/locateReportAccount.jsp">
    ...
    <jsp:include flush='true' page="/reporting/locateReportItem.jsp">
    <jsp:param name="jspFormAction"            value="/userportal/custAcctMgtReportLocDateSelect.do"/>
    <jsp:param name="jspFormName"              value="CUSTOMER_REPORT_FORM"/>
    <jsp:param name="jspSubmitIdent"           value=""/>
    <jsp:param name="jspReturnFilterProperty"  value="itemFilter"/>
    <jsp:param name="checkBoxShowInactive"     value="hide"/>
    <jsp:param name="jspHtmlFormId"            value="LOCATE_ITEM_FORM_ID"/>
    <jsp:param name="jspReportIdName"          value="id"/>
    <jsp:param name="jspReportIdValue"         value="1"/>
    <jsp:param name="jspDataSourceType"        value=""/>
</jsp:include>
    ...
    <%
     GenericReportControlViewVector grcVV = theForm.getReportControls();
       if(grcVV!=null && grcVV.size()>0) {
        GenericReportControlView showInactiveSitesControl = GenericReportControlView.createValue();
        showInactiveSitesControl.setName("showInactiveSites");
        grcVV.add(showInactiveSitesControl);
        int showInactiveSitesControlInd = grcVV.size() - 1;
        String showInactiveSitesControlEl =  "reportControlValue["+showInactiveSitesControlInd+"]";
...
<% // locate item
  } else if ("LOCATE_ITEM_MULTI_OPT".equalsIgnoreCase(name) ){
    rendered = true;
    ItemViewVector itemVwV = (ItemViewVector) session.getAttribute("Report.parameter.items");
    String controlLabel = ("LOCATE_ITEM_MULTI_OPT".equalsIgnoreCase(name)) ? "admin.button.locateItem" :"";
    %>
<br/>
  ...
  <!-- Item_Multi_Opt -->
	...
	int itemCount = itemVwV.size();
    %>
	<b><%=itemCount%> items selected:</b>
<div class="subheadergeneric">
    <%
    ...
    <nobr>
      <input type="submit" property="action" class="text"
           onclick="setAndSubmit('repParams','command','Clear Items')"
           value="<app:storeMessage key="report.control.clearItems" />"/>&nbsp;
      <input type="submit" property="action" class="text"
           onclick="setAndSubmit('repParams','command','Locate Item')"
           value="<app:storeMessage key="<%=controlLabel%>" />"/>
     </nobr>
    <%
        } else {
...
<% if( (l_rpttype.indexOf("Order Totals") >= 0  || l_rpttype.indexOf("Manufacturer Item Summary") >= 0) && 
        (siteVwV==null || siteVwV.size()==0)) { %>
    <app:storeMessage  key="report.text.showInactiveSites"/>&nbsp;<html:checkbox property="<%=showInactiveSitesControlEl%>" />
<% } %>
...
<html:hidden property="showAccount" value="true"/>
