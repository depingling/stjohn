...
import com.cleanwise.view.forms.OrderOpTemporaryPoForm;
 ...
 * @author durval
 */
public class OrderOpLogic
{
    private static final Logger log = Logger.getLogger(OrderOpLogic.class);
                             ...
                             throws Exception
    {
        ...
        HttpSession session = request.getSession();
        OrderOpItemSearchForm sForm = (OrderOpItemSearchForm)session.getAttribute(
                                              "ORDER_OP_ITEM_SEARCH_FORM");
                                ...
                                throws Exception
    {
        ...
        HttpSession session = request.getSession();
        OrderOpItemDetailForm sForm = (OrderOpItemDetailForm)session.getAttribute(
                                              "ORDER_OP_ITEM_DETAIL_FORM");
...
try {
   ...
   for(int ii=0; ii<itemStatusDescV.size(); ii++){
    ...
    int CLEANWISE_STORE_ID = 1;
    int CLEANWISE_STORE_CATALOG_ID = 4;
    if (appUser.getUserStore().getStoreId() == CLEANWISE_STORE_ID) {
        if (itemStatusDescV != null && itemStatusDescV.size() > 0) {
            Iterator it = itemStatusDescV.iterator();
            while (it.hasNext()) {
                OrderItemDescData itemDescD = (OrderItemDescData) (it.next());
                if (itemDescD != null) {
                    ProductData productD = catalogInfoEjb.getCatalogClwProduct(CLEANWISE_STORE_CATALOG_ID, itemDescD.getOrderItem().getItemId());
                    itemDescD.setCatalogItemSkuNum(productD.getCatalogSkuNum());
                }
            }
        }
    }
        ...
        for(int ii=0; ii<orderPropertyDetailVec.size(); ii++){
        	OrderPropertyData note = (OrderPropertyData)orderPropertyDetailVec.get(ii);
     	   if (note.getShortDesc().equals("Split Order") || note.getShortDesc().equals("Cancelled") || note.getShortDesc().equals("Replacement")){
     		   detailForm.getCustomerOrderNotes().add(note);
     	   }
        }
...
return lUpdateErrors;
    ...
    public static void getOrderNotes(HttpServletRequest request,
                                     ...
                                     String orderItemId, String noteId)
                              throws Exception
    {
        HttpSession session = request.getSession();
        APIAccess factory = (APIAccess)session.getAttribute(
        ...
        if (null != orderItemId && !"".equals(orderItemId)){
	        orderNotes = orderEjb.getOrderItemPropertyCollection
        ...
        if (null != noteId && !"".equals(noteId))        {
            note = orderEjb.getOrderProperty
    ...
    private static String getOrderId(String orderId, HttpSession session){
        if (null == orderId || "".equals(orderId)){
            ...
            if (orderDetailForm != null && orderDetailForm.getOrderStatusDetail() != null && orderDetailForm.getOrderStatusDetail().getOrderDetail() != null){
    ...
    public static void addOrderNote(HttpServletRequest request,
                                    ActionForm form, String orderId,
                                    String orderItemId, String noteType)
                             throws Exception
    {
        ...
        if (null == noteType || "".equals(noteType))
    {
        ...
        if ("order".equals(noteType)) {
            if (null == orderId || "".equals(orderId)) {
                ...
                OrderOpDetailForm orderDetailForm = (OrderOpDetailForm)session.getAttribute(
                "ORDER_OP_DETAIL_FORM");
    ...
    public static void addInvoiceDistNote(HttpServletRequest request,
                                          ActionForm form,
                                          String pInvoiceDistId,
                                          String noteType)
                                   throws Exception
    {
        ...
        if (null == noteType || "".equals(noteType))
        {
            noteType = new String("invoiceDistNote");
        }

        dForm.setOrderNoteType(noteType);
        dForm.setNoteId(pInvoiceDistId);
        session.setAttribute("ORDER_OP_NOTE_FORM", dForm);
    }
                                 ...
                                 throws Exception
    {
        ...
        OPD.setShortDesc("Order has been resubmitted to Lawson");
        OPD.setValue("Order has been resubmitted to Lawson");
                                 ...
                                 throws Exception
    {
        ...
        else if ("invoiceDistNote".equals(noteType))
        {
            ...
            String noteId = sForm.getNoteId();
            OPD.setInvoiceDistId(Integer.parseInt(noteId));
        }
    ...
    public static ActionErrors updateCustomerNoteByOrderId(HttpServletRequest request,
    	    ActionForm form, String orderId)throws Exception{
    	
    	        log.info("SVC orderId = " + orderId);
    	        ...
    	        int orderStatusId = Integer.parseInt(orderId);
    	        Order orderEjb = factory.getOrderAPI();
    	        OrderPropertyData OPD = OrderPropertyData.createValue();
    	        OPD.setOrderId(orderStatusId);
    	        OPD.setValue(sForm.getCustomerComment());
	...
	*/
		...
		if (true == contractChangedFlag)                    {
			...
			if (null != cItemPriceViewD &&
                ...
                if (productBundle != null) {
                    ...
                    log.info("updatePendingApprovalOrder2()=> set price: " + priceValue + " for item: " + orderItemD.getItemId());
                    if (priceValue != null) {
			...
			ContractItemData contractItemD =
                ...
                if (null != contractItemD) {
                    if (productBundle != null) {
                        ...
                        log.info("updatePendingApprovalOrder2()=> fetch the distItemCost and custContractPrice : " + priceValue + " for item: " + contractItemD.getContractItemId());
                        if (priceValue != null) {
                            orderItemD.setCustContractPrice(priceValue.getValue());
                            orderItemD.setDistItemCost(priceValue.getValue());
                        } else {
                           ...
                           throws Exception
    {
        ...
        OrderItemDescDataVector itemStatusDescV = orderEjb.getOrderItemDescCollection(Integer.parseInt(
                                                                                              orderStatusId),
                                                                                      null);
        OrderOpTemporaryPoForm detailForm = (OrderOpTemporaryPoForm)form;
        detailForm.setOrderStatusDetail(orderStatusDetail);
        detailForm.setOrderItemDescList(itemStatusDescV);
                                         ...
                                         throws Exception
    {
        ...
        HttpSession session = request.getSession();
        OrderOpTemporaryPoForm sForm = (OrderOpTemporaryPoForm)form;
        APIAccess factory = (APIAccess)session.getAttribute(
                                    Constants.APIACCESS);
                                ...
                                throws Exception
    {
        ...
        SiteData sd = siteEjb.getSite(order.getOrder().getSiteId(),0,false, SessionTool.getCategoryToCostCenterView(session, siteId));
...
*/
                                                    ...
                                                    throws Exception
    {
        ...
        // only orders sent to Lawson could be
   ...
   OrderStatusDescData orderDesc ) {
    ...
    public static ActionErrors addShippedTrackingNum(HttpServletRequest request,
    	    ActionForm form)throws Exception{
    	        ActionErrors lUpdateErrors = new ActionErrors();
    	        HttpSession session = request.getSession();
    	        APIAccess factory = (APIAccess)session.getAttribute(Constants.APIACCESS);
    	        ...
    	        OrderOpDetailForm sForm = (OrderOpDetailForm)form;
    	        session.setAttribute("ORDER_OP_DETAIL_FORM", sForm);
    	        int shippedOrderItemId = Integer.parseInt(request.getParameter("shippedOrderItemId"));
    	        String shippedQtyS = request.getParameter("shippedQty");
    	        String shippedTrackingNum = request.getParameter("shippedTrackingNum");
    	        int shippedQty = 0;
    	        if (sForm != null){
    	            if (!Utility.isSet(shippedQtyS)){
    	                lUpdateErrors.add("shippedQty",
    	                                  new ActionError("variable.empty.error", "Qty"));
    	            }else{
    	            	try {
    	            		shippedQty = Integer.parseInt(shippedQtyS);
    	            		if (shippedQty <= 0){
    	            			lUpdateErrors.add("shippedQty",
  	                                  new ActionError("error.simpleGenericError", "Qty must be greater than 0"));
    	            		}
    	            	}catch(NumberFormatException e){
    	            		lUpdateErrors.add("shippedQty",
	                                  new ActionError("error.simpleGenericError", "Qty need to be number"));
    	            	}
    	            }
    	            
    	            if (!Utility.isSet(shippedTrackingNum)){
    	                lUpdateErrors.add("shippedTrackingNum",
    	                                  new ActionError("variable.empty.error", "Tracking Number"));
    	            }
    	        }
    	        ...
    	        Order orderEjb = factory.getOrderAPI();
    	        for(Iterator iter=sForm.getOrderStatusDetail().getOrderItemList().iterator(); iter.hasNext();) {
    	            OrderItemData orderItemD = (OrderItemData) iter.next();
    	            int orderItemId = orderItemD.getOrderItemId();
    	            if (orderItemD.getOrderItemId() == shippedOrderItemId){
    	            	Date currentDate = new Date();
    	    	        OrderItemActionData shippedAction = OrderItemActionData.createValue();
    	    	        shippedAction.setOrderId(orderItemD.getOrderId());
    	                shippedAction.setOrderItemId(orderItemD.getOrderItemId());
    	                shippedAction.setAffectedSku("" + orderItemD.getItemSkuNum());
    	                shippedAction.setQuantity(shippedQty);                
    	            	shippedAction.setActionDate(currentDate);
    	                shippedAction.setActionTime(currentDate);
    	                shippedAction.setAddBy(appUser.getUserName());
    	                shippedAction.setActionCd(RefCodeNames.ORDER_ITEM_DETAIL_ACTION_CD.DIST_SHIPPED);
    	                orderEjb.updateOrderItemAction(shippedAction);
    	                
    	                OrderItemActionData trackingAction = OrderItemActionData.createValue();
    	    	        trackingAction.setOrderId(orderItemD.getOrderId());
    	                trackingAction.setOrderItemId(orderItemD.getOrderItemId());
    	                trackingAction.setAffectedSku("" + orderItemD.getItemSkuNum());
    	                trackingAction.setQuantity(shippedQty);                
    	            	trackingAction.setActionDate(currentDate);
    	                trackingAction.setActionTime(currentDate);
    	                trackingAction.setAddBy(appUser.getUserName());
    	                trackingAction.setActionCd(RefCodeNames.ORDER_ITEM_DETAIL_ACTION_CD.TRACKING_NUMBER);
    	                trackingAction.setComments(shippedTrackingNum);
    	                trackingAction.setAffectedTable(OrderItemActionDataAccess.CLW_ORDER_ITEM_ACTION);
    	                trackingAction.setAffectedId(shippedAction.getOrderItemActionId());
    	                orderEjb.updateOrderItemAction(trackingAction);
    	                break;
    	            }
    	        }    	        
    ...
    public static String getOrderIdByWebOrderNum(HttpServletRequest request, String webOrderNum, int siteId)
    throws Exception
    {
    	HttpSession session = request.getSession();
    	APIAccess factory = (APIAccess)session.getAttribute(Constants.APIACCESS);
    	...
    	Order orderEjb = factory.getOrderAPI();
    	OrderStatusCriteriaData searchCriteria = OrderStatusCriteriaData.createValue();
    	searchCriteria.setWebOrderConfirmationNum(webOrderNum);
    	if (siteId>0)
    		searchCriteria.setSiteId(siteId+"");
    	else{
    		searchCriteria.setUserId(appUser.getUserId()+"");
    		searchCriteria.setUserTypeCd(appUser.getUser().getUserTypeCd());
    	}
    	OrderDataVector orderDV = orderEjb.getOrderStatusCollection(searchCriteria);
    	if (orderDV.size() > 0)
    		return ((OrderData)orderDV.get(0)).getOrderId()+"";
    	else{
    		throw new Exception("No order found for web order num: " + webOrderNum);
    	}
    }
}
