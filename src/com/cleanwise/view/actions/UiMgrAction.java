...
import com.cleanwise.view.logic.UiMgrLogic;
import com.cleanwise.view.forms.UiMgrForm;


public class UiMgrAction extends ActionSuper {
    ...
    private ActionForward processAction(String action,
        ...
        if (INIT.equals(action)) {
            ...
            ae = UiMgrLogic.init((UiMgrForm) form, request);
            if (ae.size() > 0) {
                saveErrors(request, ae);
                return mapping.findForward(ERROR);
            }

            ae = UiMgrLogic.detail((UiMgrForm) form, request);
            if (ae.size() > 0) {
                saveErrors(request, ae);
                return mapping.findForward(ERROR);
            }
            ...
            ae = UiMgrLogic.save((UiMgrForm) form, request);
            if (ae.size() > 0) {
                saveErrors(request, ae);
                return mapping.findForward(ERROR);
            }
