 ...
 */
public final class ServiceProviderMgtReportLocDateSelectAction extends ActionSuper {

    public final static String className = "ServiceProviderMgtReportLocDateSelectAction";
    // ----------------------------------------------------- Public Methods
    ...
    public ActionForward performSub(
        ...
        try {
            ...
            if (action.equals("init")) {
                ActionErrors ae = ServiceProviderMgtReportLogic.initLocDateSelect(request, form);
                if (ae.size() > 0) {
                ...
                String target = ServiceProviderMgtReportLogic.getNextMappingOfLocDateSelect(request, form);
                return (mapping.findForward(target));
            }else if("DownloadToExcel".equals(action)){
            	String reportTypeCd = request.getParameter("reportTypeCd");

      		  if(form instanceof ServiceProviderMgtReportForm){
      			  ((ServiceProviderMgtReportForm)form).setReportTypeCd(reportTypeCd);
      		  }
			  String dateF = ClwI18nUtil.getDatePattern(request);
			  ReportRequest rr2 = new ReportRequest(st.getUserData(),mr,dateF);
			  ActionErrors ae = ServiceProviderMgtReportLogic.getCustomerReport
			  ...
			  if (ae.size() > 0) {
				  ...
				  //return (mapping.findForward("success"));
				  ServiceProviderMgtReportLogic.downloadGenericReport(request,response,form, rr2);
				  return null;
			  }
			  ...
			  if (ae.size() > 0) {
				  ...
				  //return (mapping.findForward("success"));
				  ServiceProviderMgtReportLogic.downloadPDFReport(request,response,form, rr1);
				  return null;
			  }
                ...
                ActionErrors ae = ServiceProviderMgtReportLogic.returnSelected(request, form);
                if(ae.size()>0) {
                    saveErrors(request,ae);
                ...
                ActionErrors ae = ServiceProviderMgtReportLogic.clearSites(request, form);
                if(ae.size()>0) {
                    saveErrors(request, ae);
                    return mapping.findForward("failure");
                }
                ...
                ActionErrors ae = ServiceProviderMgtReportLogic.getCustomerReport(request, response, form, rr);

                response.setContentType("application/xml");
                response.setHeader("Cache-Control", "no-cache");

                Element rootEl =ServiceProviderMgtReportLogic.generateReportXmlResponse(request,ae,form);
                ...
                ActionErrors ae = ServiceProviderMgtReportLogic.checkLocationSelect(request, form);
                if (ae.size() > 0) {
                    saveErrors(request, ae);
                    return (mapping.findForward("display"));
                }
