...
function viewPrinterFriendly(oid) {
  var loc = "printerFriendlyOrder.do?action=orderStatus&showSize=n&showMfg=n&showMfgSku=n&orderId=" + oid;
...
<% if(orderStatusDetail.getOrderDetail().getAddBy() != null && !orderStatusDetail.getOrderDetail().getAddBy().equals("inv_order")) { %>
...
<% }} %>
    ...
    if(actiondate == null){
     ...
     String bgColor = ClwCustomizer.getEvenRowColor(session,(Integer)oidx);
        %>
...
<logic:greaterThan value="0" name="action" property="quantity">
 ...
 <tr bgcolor="<%=bgColor%>">
  ...
  <%if(RefCodeNames.ORDER_ITEM_DETAIL_ACTION_CD.DIST_SHIPPED.equals(actioncd)){%>
	<app:storeMessage key="order.detail.actionCode.Shipped"/>
  <%}else if(RefCodeNames.ORDER_ITEM_DETAIL_ACTION_CD.ACK_BACKORDERED.equals(actioncd)){%>
	<app:storeMessage key="order.detail.actionCode.ACK Backordered"/>
  <%}else{%>
	<%=actioncd%>
  <%}%>
        <%if(actiondate != null){%>
...
<%
if(RefCodeNames.ORDER_ITEM_DETAIL_ACTION_CD.DIST_SHIPPED.equals(actioncd)) {
    int actionId = action.getOrderItemActionId();
    OrderItemActionDataVector oiaDV = item.getOrderItemActionList();

    for(int aa=0; aa<oiaDV.size(); aa++) {
        OrderItemActionData oiaD = (OrderItemActionData) oiaDV.get(aa);
    ...
    String country = loc.getCountry();
        ...
        if(RefCodeNames.ORDER_ITEM_DETAIL_ACTION_CD.TRACKING_NUMBER.equals(actCd) &&
           "CLW_ORDER_ITEM_ACTION".equalsIgnoreCase(oiaD.getAffectedTable()) &&
           actionId==oiaD.getAffectedId()) {
  ...
  <logic:equal name="ORDER_OP_DETAIL_FORM" property="orderHasInventoryItem" value="true">
    ...
    <%
     for(int t=0; t<lastToken.size(); t++){ %>
        <td>&nbsp;&nbsp;&nbsp;-&nbsp; <a href="http://www.fedex.com/Tracking?ascend_header=1&clienttype=dotcom&cntry_code=<%=country%>&language=<%=language%>&tracknumbers=<%=lastToken.get(t)%>"><%=oiaD.getComments()%></a></td>
    <% } %>
    <%if(resaleItemsAllowed){%><td></td><%}%>
    <app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.RECEIVING%>"><td>&nbsp;</td>
    </app:authorizedForFunction>
...
<%
String myOrderStatus = com.cleanwise.view.utils.ShopTool.xlateStatus(orderStatusDetail);
boolean myShipFlag = false;
myShipFlag = orderStatusDetail.isShipped();

if( (myOrderStatus.equals("Shipped")) && (myShipFlag==true) ) { %>
  <td>Complete
<% } else if(myOrderStatus.equals("Ordered")) { %>
  <td> In Progress
         <% }else{ %>
