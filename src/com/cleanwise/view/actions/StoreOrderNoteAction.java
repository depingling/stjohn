 ...
 */
public final class StoreOrderNoteAction extends ActionSuper {
    private static final Logger log = Logger.getLogger(StoreOrderNoteAction.class);
  ...
  public ActionForward performSub(
    ...
    try {
        ...
        if (action.equals("view")) {
            StoreOrderLogic.getOrderNotes(request, form, orderId, orderItemId, noteId);
            return (mapping.findForward("display"));
        } else if (action.equals("add")) {
	    	log.info ("note action: 1" );
            StoreOrderLogic.addOrderNote(request, form, orderId, orderItemId, noteType);
            return (mapping.findForward("detail"));
        } else if (action.equals(backStr)) {
            return (mapping.findForward("close"));
        } else if (action.equals("saveNote") || action.equals(saveStr)){
	    	log.info ("note action: 2.1" );
            ActionErrors ae = StoreOrderLogic.saveNote(request, form);
