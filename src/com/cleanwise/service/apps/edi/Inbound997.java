 ...
 *@author Deping
 */
public class Inbound997 extends InboundEdiSuper
{
	private static final Logger log = Logger.getLogger(InboundEdiSuper.class);
    ...
    public void extract()
    {
        ...
        if (table != null)
        {
            ...
            if (errors.size() > 0)
                ediHandler.log.error(getAckErrorReport());
        }
        ediHandler.getReqInterchange().addTransactionDetails(mEdi997DV);
    }

    public void processTransaction()
    {
        ediHandler.appendIntegrationRequests(getAck997s());
    }
    ...
    String getAckErrorReport()
    {
        ...
        for (int i = 0; i < errors.size(); i++) {
            ...
            if (err997.getAckCode().equals("E"))
                str += InterchangeSuper.status[InterchangeSuper.ACKE_MSG] + "\n";
            else if (err997.getAckCode().equals("R"))
                str += InterchangeSuper.status[InterchangeSuper.ACKR_MSG] + "\n";
    ...
    {
        String inFilename = "c:\\temp\\production_edi\\p997_20071030162826.txt";

        InboundTranslate translator = new InboundTranslate();
        try{
            InboundTransaction inbound = translator.translateFileByHandlerName(inFilename, InboundEdiHandler.class.getName());
            if (!inbound.isFail()){
            	translator.processIntegrationRequests(inbound.getRequestsToProcess());
            }
            log.info(inbound.getTranslationReport());
            log.info("************** End of Program ****************");
        }catch(Exception e){
            translator.logError("Inbound translation failed for inbound file - " + inFilename);
        }
    }
}
