...
<html:form styleId="429" name="MANUF_SEARCH_FORM"
action="/storeportal/manuflocate.do" focus="searchField"
...
<table ID=430 width="769" border="0"  class="mainbody">
...
<td>
    ...
    <a ID=435 href="javascript:void(0);" onclick="<%=onClick%>">
...
<td><bean:write name="arrele" property="busEntity.busEntityStatusCd"/></td>
  ...
  <pg:index>
    Result Pages:
    <pg:prev>&nbsp;<a ID=436 href="<%= pageUrl %>&feedField=<%=feedField%>&feedDesc=<%=feedDesc%>&returnFormNum=<%=returnFormNum%>&submitFl=<%=submitFl%>">[<< Prev]</a></pg:prev>
    <pg:pages><%
      ...
      if (pageNumber == pagerPageNumber) {
        ...
        %><a ID=437 href="<%= pageUrl %>&feedField=<%=feedField%>&feedDesc=<%=feedDesc%>&returnFormNum=<%=returnFormNum%>&submitFl=<%=submitFl%>"><%= pageNumber %></a><%
      }
    %>
    </pg:pages>
    <pg:next>&nbsp;<a ID=438 href="<%= pageUrl %>&feedField=<%=feedField%>&feedDesc=<%=feedDesc%>&returnFormNum=<%=returnFormNum%>&submitFl=<%=submitFl%>">[Next >>]</a></pg:next>
    <br></font>
  </pg:index>
</pg:pager>
</logic:greaterThan>
