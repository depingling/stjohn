/*
 * StoreBlanketPoNumLogic.java
 *
 * Created on November 19, 2008, 13:30 PM
 */
...
import com.cleanwise.view.forms.StoreBlanketPoNumForm;
...
/**
 * Encapsulates all of the logic necessary to manage the Blanket POs (Purchase Orders)
 * @author scher
 */
public class StoreBlanketPoNumLogic {

    /**
     *initializes the configuration
     */
    public static void initConfig(HttpServletRequest request,ActionForm pForm) throws Exception{
        StoreBlanketPoNumForm sForm = (StoreBlanketPoNumForm) pForm;

        String searchType = sForm.getSearchType(); //new: svc
        if(searchType==null || searchType.trim().length()==0) searchType = "nameBegins"; //new: svc
        sForm.setSearchType(searchType); //new: svc
    ...
    /**
     *initializes the blanket po search form (StoreBlanketPoNumSearchForm)
     */
    public static void init(HttpServletRequest request,ActionForm pForm) throws Exception{
        StoreBlanketPoNumSearchForm sForm = (StoreBlanketPoNumSearchForm) pForm;
    ...
    /**
     *Performs a search based on the values in the form and populates screen with the results
     */
    public static ActionErrors search(HttpServletRequest request,
        ...
        StoreBlanketPoNumSearchForm sForm = (StoreBlanketPoNumSearchForm) pForm;
        // get the value of "Show Inactive" flag from the screen
        boolean showInactiveFl = sForm.getSearchShowInactiveFl();
        ...
        sForm.setResults(custPoEJB.searchWithFilters(crit, showInactiveFl)); //correct statement
        //sForm.setResults(custPoEJB.search(crit)); //for testing purposes only
        return lUpdateErrors;
    }
    ...
    ***/
    ...
    /**
     *Initializes the form for configuring/creating a new BlanketPoNum
     *do I need to change the type of the form (BlanketPoNumForm) here ?
     */
    ...
    /**
     *Saves the BlanketPoNum contained in the form, performs some validation on the data entry
     */
    ...
    /**
     * Saves the BlanketPoNum contained in the form, performs some validation on the data entry;
     * also saves the configuration setup and attaches and unattaches from stores
     */
    public static ActionErrors saveStoreDetailAndConfig(HttpServletRequest request,ActionForm pForm) throws Exception{
        ...
        IdVector storeIds = (IdVector) appUser.getUserStoreAsIdVector();//SVC
        ...
        //return lUpdateErrors; //SVC
        if(lUpdateErrors.size() > 0){
            return lUpdateErrors;
        }

        IdVector nl = storeIds;

        int id = sForm.getBlanketPoNumDescData().getBlanketPoNumData().getBlanketPoNumId();//SVC
        custPoEJB.addStoreBusEntityAssociations(id,nl,appUser.getUserName());//SVC
    ...
    /**
     *Searches the configuration based off the form search type (account, site, stores) and the search criteria
     */
    public static ActionErrors searchConfig(HttpServletRequest request,ActionForm pForm) throws Exception{
        ...
        if(sForm.getConfigType().equals("Site")){
            ...
            if(stores != null && !stores.isEmpty()){
                ...
                allOptions.addAll(accountEJB.getAccountCollection(toFetch));
    ...
    /**
     *Searches the configuration based off the form search type (account, site, stores), the search criteria
     *and the filter(s)
     */
    public static ActionErrors searchConfigWithFilters(HttpServletRequest request,ActionForm pForm) throws Exception{
        StoreBlanketPoNumForm sForm = (StoreBlanketPoNumForm) pForm;
        boolean confShowConfguredOnlyFl = sForm.getConfShowConfguredOnlyFl();
        ...
        if(sForm.getConfigType().equals("Site")){
            ...
            if(accounts != null && !accounts.isEmpty()){
                ...
                int oneOfSiteId =(Utility.isSet(toFetch))? ((Integer)toFetch.get(0)).intValue() : 0 ;
                allOptions.addAll(siteEJB.getSiteCollection(toFetch,SessionTool.getCategoryToCostCenterView(session, oneOfSiteId )));
            }else{
                return lUpdateErrors;
            }
        ...
        //create the selectable object
        SelectableObjects so = null;
        if (confShowConfguredOnlyFl == true){ //show ONLY configured business entities: confShowConfguredOnlyFl = true)
        	so = new SelectableObjects(selectedOptions,allOptions,ClwComparatorFactory.getBusEntityComparator(), 1);
        } else { //confShowConfguredOnlyFl = false) => show all (configured and non-configured) business entities
           so = new SelectableObjects(selectedOptions,allOptions,ClwComparatorFactory.getBusEntityComparator(), 0);
        }
