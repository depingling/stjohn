...
/**
 *  <code>DistMgrLogic</code> implements the logic needed to manipulate
 *  purchasing site records.
 *
 *@author     durval
 *@created    August 8, 2001
 */
public class DistMgrLogic {
    private static final Logger log = Logger.getLogger(DistMgrLogic.class);
    private static final String DIST_STORE_MAP_CACHE = "distributor.store.map.cache";

    /**
     *Returns the store data for the distributor that is currently being managed.
     *For non system_administrator users this is simply the store that they are currently
     *managed.  For system_administrator this is the store that the distributor is tied
     *to.  This information is cached in the session.
     *@param pSession the user who is making the request session
     *@param pDistributorId the distributor id that we want the store for
     */
    public static StoreData getStoreForManagedDistributor(HttpSession pSession, int pDistributorId)
    throws APIServiceAccessException, RemoteException{
        ...
        Map storeMap = (Map) pSession.getAttribute(DIST_STORE_MAP_CACHE);
        if(storeMap == null){
            storeMap = new HashMap();
            pSession.setAttribute(DIST_STORE_MAP_CACHE,storeMap);
        }else{
        ...
        if(theStore == null){
            ...
            idv.add(new Integer(pDistributorId));
            crit.setDistributorBusEntityIds(idv);
            ...
            if(size == 0){
                ...
                throw new RuntimeException("Multiple stores for distributor id: "+pDistributorId);
            }
     ...
     */
    private static void resetStoreDistributorCache(HttpSession pSession, int pDistributorId, int pStoreId){
        if(pDistributorId == 0){
            return;
        }
	...
	throws Exception {
        ...
        session.setAttribute("Dist.found.vector", dv);
    }
	...
	initFormVectors(request);
        ...
        HttpSession session = request.getSession();
        DistMgrDetailForm sForm =
	    (DistMgrDetailForm) session.getAttribute("DIST_DETAIL_FORM");

        if (sForm == null) {
            sForm = new DistMgrDetailForm();
            session.setAttribute("DIST_DETAIL_FORM",
				 sForm);
            request.setAttribute("DIST_DETAIL_FORM",
				 sForm);
        }
		...
		sForm.setMaxInvoiceFreightAllowed
            ...
            if(dd.getInvoiceLoadingPriceModel()!=null){
                sForm.setInvoiceLoadingPriceModel(dd.getInvoiceLoadingPriceModel().toString());
            }else{
                sForm.setInvoiceLoadingPriceModel("");
            }
            if(dd.getInvoiceAmountPercentAllowanceUpper()!=null){
                sForm.setInvoiceLoadingPriceExceptionThreshold(dd.getInvoiceAmountPercentAllowanceUpper().toString());
            }else{
                sForm.setInvoiceLoadingPriceExceptionThreshold("");
            }

            if(dd.getReceivingSystemInvoiceCd()!=null){
                sForm.setReceivingSystemInvoiceCd(dd.getReceivingSystemInvoiceCd().toString());
            }else{
	    ...
	    getShipFromForDist( request, bed.getBusEntityId());
            ...
            BusEntityDataVector manufacturers = distBean.getPrimaryManufacturers(id.intValue());
            ...
            //Accounts served
            BusEntityDataVector servedAccounts = distBean.getServedAccounts(id.intValue());
	...
	if (ids != null) {
	    session.setAttribute("Dist.filter.vector", ids);
	} else {
	    session.removeAttribute("Dist.filter.vector");
	}
	...
	throws Exception {
        ...
        // Get a reference to the admin facade
        HttpSession session = request.getSession();
        DistMgrSearchForm sForm = (DistMgrSearchForm) form;

        // Reset the session variables.
        DistributorDataVector dv = new DistributorDataVector();
        session.setAttribute("Dist.found.vector", dv);
        session.setAttribute("Dist.found.total", "0");
        ...
        session.setAttribute("Dist.found.vector", dv);
        session.setAttribute("Dist.found.total",String.valueOf(dv.size()));
    }

    /**
     *Returns a distributor data vector based off the specified search criteria
     */
    static public DistributorDataVector search(HttpServletRequest request,String fieldValue,String fieldSearchType,String searchCity,
                                               String searchState,
        String searchCounty,String searchPostalCode,String searchGroupId) throws Exception{
	...
	DistributorDataVector distributors =
	    (DistributorDataVector)session.getAttribute("Dist.found.vector");
	...
	// Reset the distributor specific information.
        DistMgrDetailForm sForm =
	    (DistMgrDetailForm) session.getAttribute("DIST_DETAIL_FORM");
	request.setAttribute("DIST_DETAIL_FORM", sForm);
	session.setAttribute("DIST_DETAIL_FORM", sForm);
    }
	...
	session.setAttribute("Dist.ship_from.vector",shipFroms);
    }
	...
	DistMgrLocateShipFrom sForm =
	    (DistMgrLocateShipFrom)pForm;
        ...
        try{
            ...
            log.info("requested distributor id is not a parsable.");
        }
	...
	throws Exception {
        ...
        HttpSession session = request.getSession();
        DistMgrDetailForm sForm = new DistMgrDetailForm();
        session.setAttribute("DIST_DETAIL_FORM", sForm);
	// Reset the ship from list.
	ArrayList empty = new ArrayList();
	session.setAttribute("Dist.ship_from.vector", empty);
	...
	throws Exception {
        ...
        BigDecimal invoiceLoadingPriceExceptionThreshold = null;
        CleanwiseUser appUser = (CleanwiseUser) session.getAttribute(Constants.APP_USER);
        int storeId=0;
        ...
        if (sForm != null) {
            ...
            if (!Utility.isSet(sForm.getStoreId())) {
                ...
                try{
                    ...
                    lUpdateErrors.add("storeId", new ActionError("error.invalidAmount","storeId"));
                }
            }
            ...
            if (Utility.isSet(sForm.getInvoiceLoadingPriceExceptionThreshold())) {
                try{
                   invoiceLoadingPriceExceptionThreshold = new BigDecimal(sForm.getInvoiceLoadingPriceExceptionThreshold());
                }catch (NumberFormatException e){
        ...
        if(Utility.isSet(sForm.getHoldInvoiceDays())){
            try{
                ...
                lUpdateErrors.add("holdInvoiceDays",new ActionError("error.invalidAmount","holdInvoiceDays"));
            }
        }
	...
	dd.setMaxInvoiceFreightAllowed(sForm.getMaxInvoiceFreightAllowed());
        ...
        if (Utility.isSet(sForm.getDistributorsCompanyCode())){
        ...
        if (invoiceLoadingPriceExceptionThreshold != null){
            dd.setInvoiceAmountPercentAllowanceUpper(invoiceLoadingPriceExceptionThreshold);
        }
	...
	if (sForm.getTypeDesc().length() != 0) {
        ...
        if(!RefCodeNames.USER_TYPE_CD.SYSTEM_ADMINISTRATOR.equals(appUser.getUser().getUserTypeCd())){
            ...
            resetStoreDistributorCache(session,dd.getBusEntity().getBusEntityId(),appUser.getUserStore().getStoreId());
        }else{
            dd.setStoreId(storeId);
            resetStoreDistributorCache(session,dd.getBusEntity().getBusEntityId(),storeId);
        }
	...
	throws Exception {
		...
		return deleteErrors;
            ...
            session.removeAttribute("Dist.found.vector");
        }
	return deleteErrors;
    }
	...
	DistShipFromAddressViewVector shipFromLocations =
	    ...
	    session.getAttribute("Dist.ship_from.vector");
	...
	(HttpServletRequest request,
	 ...
	 try {
		 ...
		 session.getAttribute("freight_handler.vector");
	     if ( null == fhv) {
		 ae.add("distributor",
			new ActionError
	...
	(HttpServletRequest request,
	 ...
	 if ( null == contactType ) { contactType = ""; }
         int contactId = 0;
	...
	(HttpServletRequest request,
     ...
     throws Exception
     {
        ...
        APIAccess factory = new APIAccess();
        Distributor distBean = factory.getDistributorAPI();
        distBean.addPrimaryManufacturer (distId, mfgId, cuser);
