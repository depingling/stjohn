...
logFile=${logDir}/hhs_network_get_$logtstamp.log


cd $XDIR/edi
indir=hhs_inbound_network
...
echo " === " > ${logFile}
sh ${XFTP_DIR}/hhs_network_get.sh >> ${logFile}
...
do
    ...
    outFile="fa997_cleanwisedat${tstamp}.txt" 
    sh ${XFTP_DIR}/cwii_to_archrcv.sh $frcvd Network >> ${logFile}
    ...
    then
      sh ${XFTP_DIR}/cwii_send.sh $outFile Network >> ${logFile}
      sh ${XFTP_DIR}/hhs_network_send.sh $outFile >> ${logFile}
