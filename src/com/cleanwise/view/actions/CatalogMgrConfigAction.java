...
import com.cleanwise.view.logic.CatalogMgrConfigLogic;
 ...
 */
public final class CatalogMgrConfigAction extends ActionSuper {
	...
	String viewallStr = getMessage(mr,request,"admin.button.viewall");
		...
		CatalogMgrConfigLogic.init(request, form);
                return (mapping.findForward("success"));
            }
            else if (action.equals(searchStr) || action.equals(viewallStr)) {
		CatalogMgrConfigLogic.search(request, form);
                return (mapping.findForward("success"));
            }
            else if (action.equals(viewallStr)) {
		CatalogMgrConfigLogic.getAll(request, form);
                return (mapping.findForward("success"));
            }
            else if (action.equals(saveStr)) {
		ae = CatalogMgrConfigLogic.update(request, form); 
                if(ae.size() > 0){
		...
		CatalogMgrConfigLogic.sort(request, form);
                return (mapping.findForward("success"));
            }
            else if (action.equals(assignDistStr)) {
		CatalogMgrConfigLogic.updateItemDist(request, form);
                return (mapping.findForward("success"));
            }
            else {
                CatalogMgrConfigLogic.init(request, form);
