...
//  SiteDeliveryDataVector dataV  = (SiteDeliveryDataVector)(session.getAttribute(Constants.NEXT_DELIVERY_DATA));
  ...
  if (nextDeliveryData != null){
    cutoffTimeS = sdfTime.format(nextDeliveryData.getCutoffSiteTime());
    cutoffDateS = ClwI18nUtil.formatDate(request,nextDeliveryData.getCutoffSiteTime(),DateFormat.FULL);
    ...
    if (nextDeliveryData.getYear() != 0 && nextDeliveryData.getWeek()!= 0 && nextDeliveryData.getDeliveryDay()!= 0 ){
      nextDeliveryDateS = ClwI18nUtil.formatDate(request,Utility.calculateDate(nextDeliveryData.getYear(), nextDeliveryData.getWeek(),
                                   nextDeliveryData.getDeliveryDay()),DateFormat.FULL);
    }
   }
  }
%>
...
<%-- 3-nd Column : SubDistributor --%>
<td  width="25%" valign="top" align = "right">
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
     <td align = "left">
        <b><app:storeMessage key="shop.header.text.servicingAgent"/></b>
        <br>
        <%=distrShortName%>
        <br>
        <%=distrAddress%>
        <br>
        <app:storeMessage key="shop.header.text.servicingAgentPhone"/>&nbsp;<%=phone%>
      </td>
     </tr>
  </table>
</td>
