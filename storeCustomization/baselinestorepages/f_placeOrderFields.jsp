...
<% if (showPlaceOrderFields && showPlaceOrderButton){%>
          <tr>
      <td colspan="6">
      <% if ( appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.PLACE_ORDER_REQUEST_SHIP_DATE) || 
    			  appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.PLACE_ORDER_MANDATORY_REQUEST_SHIP_DATE)){
      %>
        <b><app:storeMessage key="shop.checkout.text.requestedDelivery"/></b>
        (<%=ClwI18nUtil.getUIDateFormat(request)%>)<%if(appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.PLACE_ORDER_MANDATORY_REQUEST_SHIP_DATE)){%><font color="red">*</font><%}%><br>
        <!--
...
<%}%>
    ...
    <td width="33%"><!-- crc area -->
      <table cellpadding="0" cellspacing="0">
        ...
        <%
          ...
          boolean isaCRCManager     = userType.equals(RefCodeNames.USER_TYPE_CD.CRC_MANAGER) || crcManagerFl;
        %>
            ...
            <app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.EXCLUDE_ORDER_FROM_BUDGET%>">
              <tr>
                <td class="text" colspan=2>
                  ...
                  <b><app:storeMessage key="shop.budgetInfo.text.oderWillNotAffectBudget"/></b>
                </td>
              </tr>
            </app:authorizedForFunction>
        </logic:equal>
  ...
  <%-- } else if(appUser.getCreditCardFlag() &&    f_showCC && f_paymetrics) {  --%> <!-- Credit Card of PayMetrics-->
  ...
  <tr>
    ...
    <td class="text" valign="top" align="left" colspan=2>
      ...
      <%if(!f_paymetricsCC){%>
        <html:text name="CHECKOUT_FORM" property="comments" size="65" maxlength="65" value="<%=newcomments%>" />
      <%} else { %>
    ...
    <td class="text"><b><app:storeMessage key="shop.checkout.text.customerComment" arg0="<%=accountName%>" /></b><br>
      <%if(!f_paymetricsCC){%>
        <html:textarea name="CHECKOUT_FORM" property="customerComment" rows="5" cols="35"/>
      <%} else { %>
  ...
  <% if (appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.WORK_ORDER_APPROVER) ||
    ...
    <tr>
        <td class="text" valign="top" align="left" colspan="2">
        <b><app:storeMessage key="userWorkOrder.text.workOrderNumber"/>:</b><br>
        <% if(!f_paymetricsCC) {
             if (shoppingCart.getWorkOrderItem() == null) { %>
               <html:text name="CHECKOUT_FORM" property="workOrderNumber" size="20" maxlength="22" /><br>
          <% } else {%>
               <bean:write name="CHECKOUT_FORM" property="workOrderNumber" /><br>
         <%  }
           } else {%>
               <bean:write name="CHECKOUT_FORM" property="workOrderNumber" /><br>
        <%}%>
  ...
  <logic:iterate id="siteField"
	...
	<logic:present name="siteField" property="value">
    <tr>
      <td class="text"><b>
