...
public class Inbound810Checker extends ClientServicesAPI{
	...
	System.out.println("debug=" + debug);
        ...
        try{
            ...
            distMap = new HashMap();
            while(rs.next()){
                String reciever = rs.getString("interchange_receiver");
                List l = (List) distMap.get(reciever);
                if(l == null){
                    l = new ArrayList();
                    distMap.put(reciever,l);
                }
                l.add(new Integer(rs.getInt("bus_entity_id")));
            }
            File[] files = null;

	    logDebug(" List files for " + baseDir.getName());
		...
		logDebug(" scan file " + files[i].getName());
                scan(files[i]);
            }
            check();
            ...
            if(it.hasNext()){
                ...
                char delim = '\t';
                System.out.println("Mod Date" + delim + "File Name" + delim + "Dist Id" + delim + "Invoice");
            }
            while(it.hasNext()){
    ...
    private void scan(File pFile){
        ...
        logDebug("Scaning file: "+pFile.getName());
        try{
            ...
            if(filtering){
                if(!distFilterList.containsAll(distIds)){
                    //logDebug.println("Skipping file: " +pFile.getName()+" as dist id " +distId+" is not in filter");
                    return;
                }
            }
            List invoices;
            ...
            logDebug("Using seperator: "+tok810);
            ...
            System.out.println("Exception found for file: "+pFile.getName());
            e.printStackTrace(System.out);
        }
    }
    ...
    private void check() throws Exception{
        String sql = "select invoice_num from clw_invoice_dist where bus_entity_id = ? and invoice_num = ?";
        ...
        while(kit.hasNext()){
            ...
            while(eit.hasNext()){
                ...
                if(!Utility.isSet(distIdStr)){
                    ...
                    logDebug("distIdStr is NOT null: "+distIdStr);
	...
	if ( null != tdbg && tdbg.equals("y")) {
        ...
        System.out.println("Processing Directory: "+baseDir.getAbsolutePath());
