/*
 * PdfCatalogProduct.java
 *
 * Created on July 13, 2007, 2:45 PM
 */
 ...
 * @author  bstevens
 */
public class PdfCatalogProduct extends PdfSuper {
  ...
  private static final Logger log = Logger.getLogger(PdfCatalogProduct.class);
  ...
  public PdfCatalogProduct() {
  }

  public PdfCatalogProduct(HttpServletRequest pRequest, String pCatalogLocaleCd) {
    super();
    init(pRequest, pCatalogLocaleCd);
  }
  ...
  public void init(HttpServletRequest pRequest, String pCatalogLocaleCd) {
    ...
    public Locale getCatalogLocale() {
        return  mCatalogLocale;
    }
...
//    itmTbl.setWidthPercentage(100);
  ...
  private void drawHeader(Document document,
    ...
    if(printXpedxCols){
    	String qtyOnHandStr =
          ...
          String maxQtyStr =
        	  ClwI18nUtil.getMessage(mRequest,"shop.og.table.header.maxQty", null);
  ...
  public void generatePdf(UserShopForm sForm, List pItems, StoreData pStore,
    ...
    try {
      ...
      String fileName = ClwCustomizer.getAbsFilePath(mRequest,"config.txt");
...
//      if (invItems) {
      ...
      if (nonInvItems) {
        ...
        for (int i = 0; i < pItems.size(); i++) {
          ...
          PdfPTable itmTable = makeItemElement(sci);
