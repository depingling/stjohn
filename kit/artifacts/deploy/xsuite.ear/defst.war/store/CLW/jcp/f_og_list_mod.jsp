...
<bean:define id="theForm" name="<%=jspFormName%>"  type="com.cleanwise.view.forms.UserShopForm"/>
 ...
 <html:form styleId="og" name="SHOP_FORM" action="/store/shop.do">
...
<table cellpadding="0" cellspacing="0" align="center"  class="tbstd" width="100%" >
    ...
    <logic:equal name="SHOP_FORM" property="allowPurchase" value="true">
     <jsp:include flush="true" page="<%=ClwCustomizer.getStoreFilePath(request,\"f_addtocart_buttons.jsp\")%>">
        <jsp:param name="commandName" value="action"/>
        <jsp:param name="formId" value="og"/>
        <jsp:param name="shopAction" value="og_addToCart"/>
        <jsp:param name="invShopAction" value="og_addToModInvCart"/>
     </jsp:include>
     </logic:equal>
        <td>
...
<%--
    <tr>
        ...
        <app:storeMessage key="shop.og.table.header.par"/>
        <%COL_COUNT++;%>
        </td>
        <td class="shopcharthead" align="center">
        ...
        <td class="shopcharthead" align="center">
        <app:storeMessage key="shop.og.table.header.manuf"/>
            <%COL_COUNT++;%>
        </td>
        <td class="shopcharthead" align="center">
        <app:storeMessage key="shop.og.table.header.manufSkuNum"/>
        <%COL_COUNT++;%>
        </td>
...
--%>
  ...
  <% if(RefCodeNames.INVENTORY_OG_LIST_UI.COMMON_LIST.equals(ogListUI))  {%>
    ...
    <jsp:include flush='true' page='<%=ClwCustomizer.getStoreFilePath(request,"f_oitems.jsp")%>'>
        ...
        <jsp:param name="colCount" value="<%=COL_COUNT%>"/>
        <jsp:param name="keyName" value="shop.og.table.header.title.regular"/>
        <jsp:param name="tabidx" value="<%=String.valueOf(numberOfIventoryItems+1)%>"/>
    </jsp:include>
