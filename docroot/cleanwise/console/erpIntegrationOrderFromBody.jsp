...
<div class="text">
  ...
  <td align="center">
  <%
     if(theForm.getReleasedOrderIds().size()>0) {
  %>
     <html:submit property="action" value="Pickup Orders"/>
  <% } else { %>
     No unprocessed released orders found
  <% } %>
  </td>
  </tr>
  </table>
  <%
  OrderDataVector orderDV = theForm.getProcessedReleasedOrders();
  String[] result = theForm.getOrderFromResults();
  String[] erpNumsArray = theForm.getOrderPoNums();
  if(orderDV!=null) {
  %>
    <table cellspacing="0" border="0" width="769" class="mainbody">
    <tr align="center">
