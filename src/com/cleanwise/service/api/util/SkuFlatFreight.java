...
public class SkuFlatFreight
{
  PriceRuleData _rule = null;
  PriceRuleDetailDataVector _ruleDetails = null;
  boolean _affectGeneralRuleFl = false;
  BigDecimal _price;
  BigDecimal _freightCap = null;
  BigDecimal _thresholdAmount = null;
  int _accountId;
  int[] _itemIds;
  OrderHandlingItemViewVector _orderItems;
  public SkuFlatFreight(PriceRuleData pRule, PriceRuleDetailDataVector pRuleDetails) 
  throws Exception
  {
    if(pRule==null) {
    ...
    for(int ii=0; ii<_ruleDetails.size(); ii++) {
      ...
      if("AFFECT_GENERAL_RULE".equals(param)){
        ...
        try{
          ...
          _thresholdAmount.setScale(2,BigDecimal.ROUND_HALF_UP);
        } catch (Exception exc) {
      ...
      else if("FREIGHT_CAP".equals(param)){
        try{
          _freightCap = new BigDecimal(val);
          _freightCap.setScale(2,BigDecimal.ROUND_HALF_UP);
        } catch (Exception exc) {
           String errorMess = "Wrong freight cap format for freight rule. Rule id="+
              pRule.getPriceRuleId()+" Freight cap="+val;
           throw new Exception(errorMess);
        }
      }
  ...
  throws Exception
  {
    ...
    for(int ii=0; ii<_orderItems.size(); ii++) {
      ...
      sumPrice = sumPrice.add(itemPrice.multiply(new BigDecimal(oiVw.getQty())));
      
    }
    BigDecimal freight = new BigDecimal(0);
    if(_thresholdAmount==null || _thresholdAmount.doubleValue()<0.001) {
      freight = new BigDecimal(qty);
      freight.setScale(2,BigDecimal.ROUND_HALF_UP);
      freight = freight.multiply(_price);
    } else {
      if(sumPrice.doubleValue()<_thresholdAmount.doubleValue()) {
        freight = new BigDecimal(qty);
        freight.setScale(2,BigDecimal.ROUND_HALF_UP);
        freight = freight.multiply(_price);
      }
    }
    if(_freightCap!=null && _freightCap.doubleValue()>0.001) {
      if(freight.doubleValue()>_freightCap.doubleValue()) {
        freight = _freightCap;
      }
    }
    return freight;
