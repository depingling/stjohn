...
<%
 ...
 boolean f_showPO = ("true".equals(request.getParameter("f_showPO")))?true:false;
 boolean allowPoEntry = ("true".equals(request.getParameter("allowPoEntry")))?true:false;
 ...
 boolean f_paymetrics = ("true".equals(request.getParameter("f_paymetrics")))?true:false;
 boolean f_paymetricsCC = ("true".equals(request.getParameter("f_paymetricsCC")))?true:false;

 String placeOrderAction = "placeOrder";
 if (f_paymetrics && f_showCC) {
   placeOrderAction = "creditCard";
 }
%>
...
<% if (showPlaceOrderFields && showPlaceOrderButton){%>
      ...
      <app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.PLACE_ORDER_REQUEST_SHIP_DATE%>">
        ...
        (<%=ClwI18nUtil.getUIDateFormat(request)%>)<br>
         ...
         <%} else { %>
         <!--
           <bean:write name="CHECKOUT_FORM" property="requestedShipDate"/>
           -->
           <app:dojoInputDate id="requestedShipDate"
                       property="requestedShipDate"
                       module="clw.CLW"
                       targets="RequestedShipDate"/>


     <img id="RequestedShipDate" src="../externals/images/showCalendar.gif"
             width=19  height=19 border=0
             onmouseover="window.status='Choose Date';return true"
             onmouseout="window.status='';return true">
           <html:hidden name="CHECKOUT_FORM" property="requestedShipDate"/>
        <%}%>
        <br>
...
<%if(f_paymetrics && f_showCC && !f_paymetricsCC) {%>
  <tr>
    <td style="padding: 2em;" align="center" colspan="3">
    <!--font style="font-size:14px; font-weight:bold; color:red;"><app:storeMessage key="shop.checkout.text.paymetricsWarning"/></font-->
    <font style="font-size:14px; font-weight:bold; color:red;">You will be asked to enter your credit card information in the next step.</font>
    </td>
  </tr>
<%}%>
  ...
  <tr>
    <td style="padding: 2em;" align="left" colspan="2">
      <%if(!f_paymetricsCC) {%>
        <jsp:include flush='true' page="f_review_items_note.jsp"/>
      <%}%>
    </td>
    <td>
      <% if (showPlaceOrderFields && showPlaceOrderButton){%>
...
<%}%>
  <%
    ...
    if ( prevOrder != null
	  ...
	  if(newpoval.length()==0 && prevOrder.getRequestPoNum()!=null){
		 newpoval = prevOrder.getRequestPoNum();
	  }
    }
  ...
  <%
	...
	*/
    ...
    if ( appUser.isaCustServiceRep() || crcManagerFl || isStoreAdmin ) {
  ...
  <tr>
    <%if(showPoField){%><td align="left" class="text" width="33%" valign="top"><%}%>
    ...
    <b><app:storeMessage key="shop.checkout.text.poNumber"/>  </b><br>
      ...
      <%} else { %>
        ...
        <html:hidden name="CHECKOUT_FORM" property="poNumber"/>
      <%}%>
    ...
    <td width="66%" valign="top" colspan="2">
      ...
      <%} else { %>
        ...
        <html:hidden name="CHECKOUT_FORM" property="processOrderDate"/>
      <%}%>
        <br>
      </app:authorizedForFunction>
    ...
    <td width="33%"><!-- crc area -->
      <table cellpadding="0" cellspacing="0">
        ...
        <td>
      <%if(!f_paymetricsCC){%>
             <html:text name="CHECKOUT_FORM" property="orderContactName" size="30" value="<%=prevOrderContactName%>" />
             <font color="red">*</font>
      <%} else { %>
      ...
      <%}%>
        ...
        <td>
      <%if(!f_paymetricsCC){%>
          <html:text name="CHECKOUT_FORM" property="orderContactPhoneNum" size="20" value="<%=prevOrderContactPhoneNum%>" />
          <font color="red">*</font>
      <%} else { %>
      ...
      <%}%>
        ...
        <td>
      <%if(!f_paymetricsCC){%>
          <html:text name="CHECKOUT_FORM" property="orderContactEmail" size="20" value="<%=prevOrderContactEmail%>" />
      <%} else { %>
      ...
      <%}%>
        ...
        <%
          boolean isaCustServiceRep = appUser.isaCustServiceRep();
          boolean isaOrderApprover  = RefCodeNames.WORKFLOW_ROLE_CD.ORDER_APPROVER.equals(clwWorkflowRole);
          boolean isaCRCManager     = userType.equals(RefCodeNames.USER_TYPE_CD.CRC_MANAGER) || crcManagerFl || isStoreAdmin;
        %>

        <% if (appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.BYPASS_SMALL_ORDER_ROUTING) ||
                (isaCustServiceRep && (isaOrderApprover || isaCRCManager))) { %>
        <tr>
            <td class="text" colspan=2>
                ...
                <%} else if (theForm.isBypassOrderRouting()) { %>
                   <b><app:storeMessage key="shop.checkout.text.bypasSmallOrderRouting"/></b>
                <%}%>
            </td>
        </tr>
        ...
        <% if (appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.BYPASS_CUSTOMER_WORKFLOW) ||
                (isaCustServiceRep && (isaOrderApprover || isaCRCManager))) { %>
        <tr>
            <td class="text" colspan=2>
                <%if(!f_paymetricsCC){%>
                ...
                <%} else if (theForm.isBypassCustomerWorkflow()) { %>
                   <b><app:storeMessage key="shop.checkout.text.bypasCustomerWorkflow"/></b>
                <%}%>
            </td>
        </tr>
        ...
        <% if (appUser.isAuthorizedForFunction(RefCodeNames.APPLICATION_FUNCTIONS.CUST_REQ_RESHIPMENT_ORDER_NUM) || isaCustServiceRep || crcManagerFl || isStoreAdmin) { %>
        <tr>
            <td class="text" colspan=2>
        ...
        <app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.PLACE_CONFIRMATION_ONLY_ORDER%>">
          <tr>
            <td class="text" colspan=2>
                ...
                <%} else if (theForm.isBillingOrder()) { %>
                   <b><app:storeMessage key="shop.checkout.text.confirmationOnlyOrder"/></b>
                <%}%>
              <br>
       ...
       <% if (appUser.getUserAccount().isShowReBillOrder()) { %>
        ...
        <logic:equal name="<%=Constants.APP_USER%>" property="site.hasBudgets" value="true">
          <app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.EXCLUDE_ORDER_FROM_BUDGET%>">
            <tr>
              <td class="text" colspan=2>
                <html:checkbox name="CHECKOUT_FORM" property="bypassBudget"/>
                <b><app:storeMessage key="shop.budgetInfo.text.oderWillNotAffectBudgetButWillRequireApproval"/></b>
              </td>
            </tr>
          </app:authorizedForFunction>
        </logic:equal>
  ...
  <tr>
    <td colspan="2" class="text"><b> <app:storeMessage key="shop.checkout.text.oderNoteInternal"/> </b><br>
     <%if(!f_paymetricsCC){%>
         <html:textarea name="CHECKOUT_FORM" property="orderNote" rows="5" cols="45"/>
     <%} else { %>
         <bean:write name="CHECKOUT_FORM" property="orderNote"/>
         <html:hidden name="CHECKOUT_FORM" property="orderNote"/>
     <%}%>
    </td>
  </tr>
  ...
  <tr>
    <td class="text" width="25%">
     ...
     <%if(!f_paymetricsCC){%>
      ...
      <font color="red">*</font>
     <%} else { %>
         <bean:write name="CHECKOUT_FORM" property="otherPaymentInfo"/>
         <html:hidden name="CHECKOUT_FORM" property="otherPaymentInfo"/>
     <%}%>
    </td>
  </tr>
  <% }/* End other payment section. */%>

  <% if(appUser.getCreditCardFlag() &&    f_showCC && !f_paymetrics) {  %>
  <tr>
  ...
  <% } else if(appUser.getCreditCardFlag() && f_showCC && f_paymetrics && f_paymetricsCC) {  %> <!-- Credit Card of PayMetrics-->
  <tr>
    <td class="text" colspan="2">
    <div>
        <html:hidden name="CHECKOUT_FORM" styleId="xiPayCCToken" property="xiPayCCToken"/>
        <html:hidden name="CHECKOUT_FORM" styleId="ccExpMonth" property="ccExpMonth"/>
        <html:hidden name="CHECKOUT_FORM" styleId="ccExpYear" property="ccExpYear"/>
        <html:hidden name="CHECKOUT_FORM" styleId="xiPayCCType" property="xiPayCCType"/>
        <html:hidden name="CHECKOUT_FORM" styleId="xiPayBinRangeType" property="xiPayBinRangeType"/>
        <iframe src='paymetricsAuth.jsp' style="border: 0px; width:350px;height:215px;" id="xiPay"></iframe>
        <!--iframe src='paymetricsAuth.jsp' style="border: 0px; width:750px;height:215px;" id="xiPay"></iframe-->
    </div>
    </td>
    <td align="left">
      <!--Table level3 begin -->
  ...
  <tr>
    ...
    <td class="text" valign="top" align="left" colspan=2>
      ...
      <%if(!f_paymetricsCC){%>
        <html:text name="CHECKOUT_FORM" property="comments" size="65" maxlength="255" value="<%=newcomments%>" />
      <%} else { %>
    ...
    <td class="text"><b><app:storeMessage key="shop.checkout.text.customerComment" arg0="<%=accountName%>" /></b><br>
      ...
      <%} else { %>
        ...
        <html:hidden name="CHECKOUT_FORM" property="customerComment"/>
      <%}%>
    <%}%> <%--is authorized for ADD_CUSTOMER_ORDER_NOTES check--%>
  </tr>

  <%-- SiteDeliveryDataVector deliveryDataCollection = (SiteDeliveryDataVector) session.getAttribute("Delivery.dates.vector"); --%>
  <%-- if ((SiteDeliveryDataVector) session.getAttribute("Delivery.dates.vector")!= null && ((ArrayList) session.getAttribute("Delivery.dates.vector")).size() >0 ){ --%>
  <% if( session.getAttribute("Delivery.dates.vector") != null  ){ %>
  <tr>
    <td class="text" valign="top" align="left" colspan=2>
      <b><app:storeMessage key="shop.checkout.text.deliveryDate"/></b>
        <html:select name="CHECKOUT_FORM" property="deliveryDate" >
          <html:options  collection="Delivery.dates.vector" property="value" />
        </html:select>
    </td>
  </tr>
  <%}%>
  ...
  <logic:iterate id="checkoutField"
    name="CHECKOUT_FORM" property="checkoutFieldProperties"
    type="com.cleanwise.service.api.value.BusEntityFieldDataElement"  indexId="i">
  	<%String prop = "checkoutFieldProperties["+i+"].value";%>
	<logic:notEmpty name="checkoutField" property="tag">
	<logic:equal name="checkoutField" property="showRuntime" value="true">
    <tr>
      <td class="text">
        <b><bean:write name="checkoutField" property="tag"/></b>
        <logic:equal name="checkoutField" property="required" value="true">
       		<span class="reqind">*</span>
       	</logic:equal>
      </td>
      <td><html:text name="CHECKOUT_FORM" property="<%=prop%>" size="20" maxlength="56"/></td>
    </tr>
    </logic:equal>
  </logic:notEmpty>
  </logic:iterate>
...
<%if(!isOrderService && !f_paymetricsCC) {%>
<table class="tbstd" width="<%=Constants.TABLEWIDTH%>"
  ...
  <tr>
    ...
    <td align=left>
      ...
      <a href="#" class="linkButton" onclick="setAndSubmit('<%=formId%>','command','<%=placeOrderAction%>');"
