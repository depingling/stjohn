...
import com.cleanwise.view.logic.StoreDistConfMgrLogic;
 ...
 * @author Ykupershmidt
 */
public final class StoreDistConfMgrAction  extends ActionSuper {
    ...
    public ActionForward performSub(
        ...
        try {
            if (action.equals("init")) {
                StoreDistConfMgrLogic.init(request, form);
                return (mapping.findForward("success"));
            }
            else if (action.equals(searchStr)) {
               StoreDistConfMgrLogic.search(request, form);
               return (mapping.findForward("success"));
            }
            else if (action.equals(saveStr)) {
                ae = StoreDistConfMgrLogic.update(request, form);
                if(ae.size() > 0){
               ...
               //StoreDistConfMgrLogic.sort(request, form);
               return (mapping.findForward("success"));
            }
            else if (action.equals(saveConfig)) {
                ae = StoreDistConfMgrLogic.updateDistributorConfiguration(request, form);
                if (ae.size() > 0) {
                    saveErrors(request, ae);
                }
