...
<!-- list of items -->
<% 
   ItemDataDocUrlsViewVector sciDV = theForm.getDedUrls();
   if(sciDV!=null && sciDV.size()>0) {
%>
<table align="left" border="0" cellpadding="0" cellspacing="0">
<tr>
<td class="smalltext"><b><app:storeMessage key="shop.msdsSpecs.text.myDisinfectantEfficacyData"/></b></td>
<td class="smalltext">
  &nbsp;
</td>
</tr>
<logic:iterate id="item" name="MSDS_FORM" property="dedUrls"
   indexId="kkk"
   type="com.cleanwise.service.api.value.ItemDataDocUrlsView">
  <tr>
  <td class="smalltext">
  <%
  String docUrl =item.getDed();
  
  String dedStorageType = item.getDedStorageTypeCd();
  ...
  <% if (dedStorageType.trim().equals(RefCodeNames.BINARY_DATA_STORAGE_TYPE.DATABASE)) { %>
         <%    String documentTypeFromDbLink = "/store/shop.do?action=itemDocumentFromDb&path=" + docUrl; 	%>
	      <html:link action="<%=documentTypeFromDbLink %>" target="_blank"><bean:write name="item" property="itemName"/>&nbsp;
	      </html:link>
  <% } %>
  <% if (dedStorageType.trim().equals(RefCodeNames.BINARY_DATA_STORAGE_TYPE.E3_STORAGE)) { %> 
