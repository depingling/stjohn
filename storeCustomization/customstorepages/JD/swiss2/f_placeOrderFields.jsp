...
function checkMaxLength(obj, maxLength) {
	if (obj.value.length > maxLength) {
		//alert('Maximum number of characters allowed = ' + maxLength);
...
<%
 ...
 int SHIPPING_COMMENTS_MAX_LENGTH = 140;
%>
...
<% if (showPlaceOrderFields && showPlaceOrderButton){%>
      ...
      <app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.PLACE_ORDER_REQUEST_SHIP_DATE%>">
        ...
        <html:text name="CHECKOUT_FORM" property="requestedShipDate" size="10" maxlength="10" />
            <a href="#" onClick="showCalendar('requestedShipDate', event); return false;"
               title="Choose Date">
...
<%}%>
  ...
  <tr>
    <%if(allowShippingCommentsEntry && !appUser.getUserAccount().hideShippingComments()){
    	String eventAction = "return checkMaxLength(this,'" + SHIPPING_COMMENTS_MAX_LENGTH + "');";
    %>
    <td class="text" valign="top" align="left" colspan=2>
      <b><app:storeMessage key="shop.checkout.text.shippingComments"/></b><br>
      <html:textarea name="CHECKOUT_FORM" property="comments" rows="5" cols="35" 
      	onkeyup="<%=eventAction%>" onblur="<%=eventAction%>"
      	value="<%=newcomments%>"/><br>
