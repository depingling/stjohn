...
public class GetFtpBulkJob extends JanPakFtpJob {

    /**
     * Quartz requires a public empty constructor so that the
     * scheduler can instantiate the class whenever it needs.
     */
    public GetFtpBulkJob() {
    }
    ...
    public void process(InboundFile file, JobDetail jobDetail) throws Exception {
        ...
        eventEjb.addEventProcess(epv, "GetFtpBulkJob");
    ...
    private ProcessSchema createProcessSchema(JobDetail jobDetail) {
        ...
        pSchema.addStep(TaskDescriptor.JAN_PACK_SITE_LOADER.PREPARE);
        pSchema.addStep(TaskDescriptor.JAN_PACK_SITE_LOADER.MATCH);
        pSchema.addStep(TaskDescriptor.JAN_PACK_SITE_LOADER.UPDATE);
        pSchema.addStep(TaskDescriptor.JAN_PACK_SITE_LOADER.INSERT);
        pSchema.addStep(TaskDescriptor.JAN_PACK_SITE_LOADER.DELETE);
        if(Utility.isTrue(jobDetail.getJobDataMap().getString(P_REPORT))) {
            pSchema.addStep(TaskDescriptor.JAN_PACK_SITE_LOADER.REPORT);
        }
    ...
    public EventProcessView createEvent(JobDetail pJobDetail, Object... pParams) throws Exception {
        ...
        ProcessData process = APIAccess.getAPIAccess().getProcessAPI().getProcessByName(RefCodeNames.PROCESS_NAMES.JAN_PACK_SITE_LOADER);
