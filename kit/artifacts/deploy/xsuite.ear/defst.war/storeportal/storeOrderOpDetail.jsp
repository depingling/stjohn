...
<head>
<title>Operations Admin Store Home: Order Status</title>
...
<html:form name="ORDER_OP_DETAIL_FORM" action="/storeportal/storeOrderOpDetail.do"
        type="com.cleanwise.view.forms.OrderOpDetailForm">
...
<jsp:include flush='true' page="f_storeOrderOpCharges.jsp">
...
<% if(clwSwitch) { %>
<% String backfillClick = new String("window.location.href='storeOrderBackfill.do?action=view&id="+orderId+"';");%>
&nbsp;<html:button property="action" value="Backfill" onclick="<%=backfillClick%>"/>
        <html:submit property="action">
            <app:storeMessage  key="button.refresh"/>
        </html:submit>
<% } %>
&nbsp;<input type="button" onclick="popLink('storeOrderOpDetail.do?action=siteNotes');" value="Site Notes" />
<logic:present name="ORDER_OP_DETAIL_FORM" property="orderStatusDetail.orderCreditCardData">
  ...
  <%if(showCC){%>
    &nbsp;<input type="button" onclick="popLink('storeOrderCreditCard.do?orderCreditCardId=<%=orderStatusDetail.getOrderCreditCardData().getOrderCreditCardId()%>&amp;action=init');" value="Credit Card" />
  <%}%>
</logic:present>
<logic:notPresent name="ORDER_OP_DETAIL_FORM" property="orderStatusDetail.orderCreditCardData">
  <app:authorizedForFunction name="<%=RefCodeNames.APPLICATION_FUNCTIONS.MODIFY_ORD_CREDIT_CARD%>">
    &nbsp;<input type="button" onclick="popLink('storeOrderCreditCard.do?orderIdS=<%=orderId%>&amp;action=create');" value="Add Credit Card" />
  </app:authorizedForFunction>
</logic:notPresent>
...
<td class="<%=style%>"><b><a href="storeOrderOpDetail.do?action=sortitems&sortField=outboundPONum">Outbound PO#</b></td>
<% } %>
<% if(clwSwitch){ %>
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
<td class="<%=style%>"><b><a href="storeOrderOpDetail.do?action=sortitems&sortField=cwSKU">CW SKU#</b></td>
<% } else { %>
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
<td class="<%=style%>"><b><a href="storeOrderOpDetail.do?action=sortitems&sortField=cwSKU">SKU#</b></td>
<% } %>
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
<td class="<%=style%>"><b><a href="storeOrderOpDetail.do?action=sortitems&sortField=distSKU">Dist SKU#</b></td>
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
<td class="<%=style%>"><b>Dist Name</b></td>
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
<td class="<%=style%>"><b><a href="storeOrderOpDetail.do?action=sortitems&sortField=name">Product Name</b></td>
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
...
<logic:iterate id="itemele" indexId="i"
 ...
 <% String linkHref = new String("storeOrderOpItemDetail.do?action=view&id=" + key  + "&fromPage=order");%>
...
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
<td class="<%=style%>"><input type="button" class="smallbutton" onclick="popLink('storeOrderOpNote.do?action=view&type=item&itemid=<%=key%>');" value="View"></td>
</logic:equal>
...
<td class="<%=style%>">
        <logic:notEqual name="itemele" property="orderItem.orderItemId" value="0">
                <input type="button" class="smallbutton" onclick="popLink('storeOrderOpNote.do?action=add&type=item&itemid=<%=key%>');" value="Add">
        </logic:notEqual>
</td>
<% style = ((colInd++)%2)==0?"resultscolumna":"resultscolumnb"; %>
<td class="<%=style%>">
...
<% String detailLinkHref = new String("storeOrderOpItemUpdate.do?action=edit&id=" + key + "&detailid=" + detailkey);%>
...
<html:submit property="action">
<app:storeMessage  key="admin.button.reorder"/>
</html:submit>
<html:submit property="action">
