 ...
 * @author  bstevens
 */
public class PdfOrderGuideThumbnail extends PdfOrderGuide {
  ...
  public PdfOrderGuideThumbnail() {
      super();
  }

  public PdfOrderGuideThumbnail(HttpServletRequest pRequest, String pCatalogLocaleCd) {
    super();
    mRequest = pRequest;
    try {
  ...
  private Table makeItemElement(ShoppingCartItemData pItm) throws DocumentException {
    ...
    if (!catalogOnly) {
      // add thumbnail
      String thumbnail = pItm.getProduct().getThumbnail();
      Image thImage = null;
      Cell thumb = null;
      if(thumbnail!=null && thumbnail.trim().length()>0) {
        String imgPath = ClwApiCustomizer.getCustomizeElement(thumbnail);
        thImage = makeImage(imgPath,50,50);
        if (thImage != null) {
            thumb = new Cell(thImage);
        } else {
            thumb = new Cell("");
        }
        thumb.setColspan(1);
//        thumb.setBackgroundColor(Color.BLUE);
 ...
 //     Cell thumb = new Cell(thPhrase);
      thumb.setBorder(Cell.NO_BORDER);
      itmTbl.addCell(thumb);
  ...
  private void initColumnsAdnWidths(CleanwiseUser appUser) {
    ...
    if (!catalogOnly) {
      itmColumnWidth[i++] = 9;     // thumbnail
      itmColumnWidth[i++] = 8;     // on hand
      itmColumnWidth[i++] = 8;     // requested qty
      itmColumnWidth[i++] = 10;    // sku
      itmColumnWidth[i++] = 32;    // product name
      if (mShowSize) {
        itmColumnWidth[i++] = 10;  // size
      }
      itmColumnWidth[i++] = 6;     // pack
      itmColumnWidth[i++] = 6;     // uom
      if (mShowPrice) {
        itmColumnWidth[i++] = 12;  // price
      }
      itmColumnWidth[i++] = 8;     // amount
  ...
  private Image makeImage(String pImageName, float xSize, float ySize) {
    try {
      Image image = Image.getInstance(pImageName);
