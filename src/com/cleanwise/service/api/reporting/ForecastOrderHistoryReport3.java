 ...
 *
 */
public class ForecastOrderHistoryReport3 implements GenericReportMulti {
       private static final Logger log = Logger.getLogger(ForecastOrderHistoryReport3.class);
	    ...
	    throws Exception
	    {
	        ...
	        if(siteIdV.size() > 1000){

                List id1 = siteIdV.subList(0, 1000);
                List id2 = null;
                if (siteIdV.size() > 2000) {
                    id2 = siteIdV.subList(1000, 2000);
                    log.info("[ForecastOrderHistoryReport3] Number of sites is more than 2000. So only first 2000 sites will be processed.");
                } else {
                    id2 = siteIdV.subList(1000, siteIdV.size());
                }
        	...
        	if(siteIdV.size() !=0 && !siteIdV.isEmpty()){
        		...
        		if(sites1 != null && sites2 !=null){
        			...
        			crit.addOneOf(BusEntityAssocDataAccess.BUS_ENTITY1_ID, sites1);
        			crit.addEqualToIgnoreCase(BusEntityAssocDataAccess.BUS_ENTITY_ASSOC_CD,
        					RefCodeNames.BUS_ENTITY_ASSOC_CD.SITE_ACCOUNT);
        			BusEntityAssocDataVector beaDV1 =
        				BusEntityAssocDataAccess.select(con, crit);
        			for(int i=0; i<beaDV1.size(); i++){
        				BusEntityAssocData beaD = (BusEntityAssocData)beaDV1.get(i);
        			...
        			crit = new DBCriteria();
        			crit.addOneOf(BusEntityAssocDataAccess.BUS_ENTITY1_ID, sites2);
        			crit.addEqualToIgnoreCase(BusEntityAssocDataAccess.BUS_ENTITY_ASSOC_CD,
        					RefCodeNames.BUS_ENTITY_ASSOC_CD.SITE_ACCOUNT);
        			BusEntityAssocDataVector beaDV2 =
        				BusEntityAssocDataAccess.select(con, crit);
        			for(int i=0; i<beaDV2.size(); i++){
        				BusEntityAssocData beaD = (BusEntityAssocData)beaDV2.get(i);
        				int acctId = beaD.getBusEntity2Id();
        				if(! acctIdV.contains(new Integer(acctId))){
        					acctIdV.add(new Integer(acctId));
        				}
	        ...
	        if(begDateS.length()!=0 && endDateS.length()!=0){
	        	...
	        	sql =
	        		...
	        		"NVL(o.cust_item_sku_num,NVL((SELECT customer_sku_num FROM clw_catalog_assoc ca, clw_catalog_structure cs WHERE cs.catalog_id = ca.catalog_id AND ca.bus_entity_id = site.bus_entity_id AND cs.item_id =i.item_id),inv.dist_item_num)) as sku_num, "+
	        		"i.short_desc, "+
	        		...
	        		"NVL(cat.short_desc,inv.category) AS category, " +
	        		"NVL(cat.cost_center,inv.cost_center) AS cost_center," +
	        		"NVL(o.uom,meta.clw_value) AS uom, "+
	        		"NVL(o.price, inv.price) AS price," +
	        		...
	        		"CLW_BUS_ENTITY site, CLW_ITEM i, CLW_ITEM_META meta, "+
	        		"clw_bus_entity_assoc ba, CLW_INVENTORY_ITEMS ii, "+
              "(SELECT itemass.item1_id, cat.short_desc,ba.bus_entity1_id as site_id, cc.short_desc AS cost_center" +
              "  FROM clw_item cat, clw_catalog c, clw_catalog_assoc cass, "+
	        		      "clw_catalog_structure cst, clw_item_assoc itemass, clw_bus_entity_assoc ba,clw_cost_center cc "+
                    "WHERE "+
                    "cat.item_id = itemass.item2_id "+
	        		      ...
	        		      "AND cc.cost_center_id = cst.cost_center_id"+
              ") cat "+
"WHERE "+
              ...
              "AND ba.bus_entity2_id IN ("+accts+") ";
	        	...
	        	if(sites1 !=null && sites2 !=null){
	        		sql +=
	        			"AND (ba.bus_entity1_id  IN ("+sites1+") "+
	        			"OR ba.bus_entity1_id IN ("+sites2+")) "+
	        			"AND inv.item_id = cat.item1_id (+) "+
	        			"AND inv.item_id = meta.item_id "+
	        			"AND meta.name_value = 'UOM' " +
...
"WHERE "+
	        ...
	        log.info("ForecastOrderHistoryReport JJJJJJJJJJJJJJJJJJJJJJJJJJJJ sql: "+sql);
	        ...
	        while(rs.next()) {
                ...
                foh.cost_center = rs.getString("cost_center");
                foh.uom = rs.getString("uom");
                foh.price = rs.getBigDecimal("price");
                foh.ext_price = rs.getBigDecimal("extended_price");
	    ...
	    private GenericReportColumnViewVector getForecastOrderHistoryReportHeader() {
            ...
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.String", "Location Name", 0, 255, "VARCHAR2","10",false));
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.String", "P/N", 0, 255, "VARCHAR2","12",false));
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.String", "Description", 0, 255, "VARCHAR2","20",false));
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.Integer", "Forecast Qty", 0,38, "NUMBER","10",false));
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.Integer", "On-hand Qty", 0,38, "NUMBER","8",false));
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.Integer", "Order Qty", 0,38, "NUMBER","8",false));
            ...
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.String", "Category", 0, 255, "VARCHAR2","15",false));
            header.add(ReportingUtils.createGenericReportColumnView("java.lang.String", "Cost Center", 0, 255, "VARCHAR2","15",false));
            ...
            header.add(ReportingUtils.createGenericReportColumnView("java.math.BigDecimal","Extended Price", 2, 20, "NUMBER","10",false));
