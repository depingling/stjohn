 ...
 * @author  bstevens
 */
public class PdfOrderStatus extends PdfSuper {
    ...
    public PdfOrderStatus() {
      super();
    }
    public PdfOrderStatus(HttpServletRequest pRequest, String pOrderLocaleCd) {
      super();
    ...
    public Table makeSubHeader(OrderItemDescData oidD) throws Exception{
        ...
        if(frtName != null){
            ...
            } else { //bug # 4795: Modified to display Handling and Discount strings in different rows in a pdf.
                  frtAmt = new BigDecimal(0);
                  pCharges.add(makePhrase(SPACE + mFormatter.priceFormat(frtAmt,SPACE),normal,true));
            }
        }

        //handling
        String handlingName=null;
        BigDecimal handlingAmt = new BigDecimal(0);
        if(oidD.getOrderHandling() != null){
        	handlingName = oidD.getOrderHandling().getShortDesc();
        	handlingAmt = oidD.getOrderHandling().getAmount();
        }
        if(handlingName != null){
            pCharges.add(makePhrase(handlingName,normalBold,false));
            if(handlingAmt != null){
                  pCharges.add(makePhrase(SPACE + mFormatter.priceFormat(handlingAmt,SPACE),normal,true));
            } 
        }
    ...
    throws DocumentException{
        ...
        if(!RefCodeNames.ORDER_ITEM_STATUS_CD.CANCELLED.
          ...
          String amountS = null;
          try {
            amountS = mFormatter.priceFormat(amount," ");
          }catch(Exception exc) {
            exc.printStackTrace();
            NumberFormat cnf = NumberFormat.getCurrencyInstance(mOrderLocale);
    ...
    throws DocumentException {
       ...
       drawLogo(appUser,document, pPageNumber, pImageName);
        ...
        String  extendedPriceStr = ClwI18nUtil.getMessage(mRequest,"shoppingItems.text.extendedPrice",null);
 ...
 /*
        ...
        PdfPTable credit = new PdfPTable(2);
        credit.setWidthPercentage(50);
        credit.setWidths(halves);
        credit.getDefaultCell().setBorder(borderType);
        credit.getDefaultCell().setHorizontalAlignment(PdfPCell.ALIGN_LEFT);

        //add any credit card info
        String ccNumber = sForm.getCcNumber();
        if(ccNumber!=null && ccNumber.trim().length()>0)
        {
            credit.addCell(makePhrase("Credit Card Information",smallHeading,false));
            credit.addCell(makePhrase("",smallHeading,true));

            String ccType = sForm.getCcType();
            String ccTypeView = "";
            credit.addCell(makePhrase("Card Type:",normal,false));
            ...
            credit.addCell(makePhrase(ccTypeView,normal,true));

            credit.addCell(makePhrase("Card Number: ",normal,false));
            credit.addCell(makePhrase(sForm.getCcNumber(),normal,true));

            credit.addCell(makePhrase("Exp. Date: ",normal,false));
            String ccExpMonthS = "";
            ...
            credit.addCell(makePhrase(ccExpMonthS+SPACE+sForm.getCcExpYear(),normal,true));

            credit.addCell(makePhrase("Name: ",normal,false));
            credit.addCell(makePhrase(sForm.getCcPersonName(),normal,true));

            credit.addCell(makePhrase("Street 1: ",normal,false));
            credit.addCell(makePhrase(sForm.getCcStreet1(),normal,true));

            credit.addCell(makePhrase("Street 2: ",normal,false));
            credit.addCell(makePhrase(sForm.getCcStreet2(),normal,true));

            credit.addCell(makePhrase("City, State, Zip: ",normal,false));
            credit.addCell(makePhrase(sForm.getCcCity()+", "+sForm.getCcState()+SPACE+
                                      sForm.getCcZipCode(),normal,true));

        }
*/
        header.addCell(orderInfo);
//        header.addCell(credit);
        document.add(header);
/*
        ...
        comments.addCell(makePhrase("Comments: "+orderD.getComments(),normal,false));
...
*/
    ...
    private Table makeSummary(OrderOpDetailForm sForm, CleanwiseUser appUser) throws Exception{
        ...
        String  totalStr =
              ClwI18nUtil.getMessage(mRequest,"shop.orderStatus.text.total:",null);
  ...
  if ( orderStatusDesc.hasCreditCard() ) {
    ...
    orderStatusDesc.getOrderCreditCardData();
      ...
      acctInfo.addCell(makePhrase(ccod.getCountryCd(), normal, true));
        ...
        String temp = shipTo.getCity();
        if (appUser.getUserStore().isStateProvinceRequired() && Utility.isSet(shipTo.getStateProvinceCd())){
        	if (Utility.isSet(temp))
        		temp += ", " + shipTo.getStateProvinceCd();
        	else
        		temp = shipTo.getStateProvinceCd();
        }
        if (Utility.isSet(shipTo.getPostalCode())){
        	if (Utility.isSet(temp))
        		temp += " " + shipTo.getPostalCode();
        	else
        		temp = shipTo.getPostalCode();
        }
        if (Utility.isSet(temp)){
        	siteInfo.addCell(makePhrase(temp, normal, true));
        }        
