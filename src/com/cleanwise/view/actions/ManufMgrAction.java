...
import com.cleanwise.view.logic.ManufMgrLogic;
import com.cleanwise.view.utils.*;


/**
 *  Implementation of <strong>Action</strong> that processes the Manuf manager
 *  page.
 *
 *@author     dvieira
 *@created    August 9, 2001
 */
public final class ManufMgrAction extends ActionSuper {
	...
	String createStr = getMessage(mr,request,"admin.button.create");
        ...
        try {
		    ...
		    ManufMgrLogic.updateManufacturer(request, form);
                ...
                ManufMgrLogic.getDetail(request, form);
                return (mapping.findForward("manufdetail"));
            }
            else if (action.equals(deleteStr)) {
                ActionErrors ae = 
		    ManufMgrLogic.delete(request, form);
                if (ae.size() > 0) {
                    saveErrors(request, ae);
		    return (mapping.findForward("manufdetail"));
                }
                ...
                ManufMgrLogic.sort(request, form);
                return (mapping.findForward("success"));
            }
            else {
                ManufMgrLogic.init(request, form);
